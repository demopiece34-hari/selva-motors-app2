...
# ============================================================
SHEETS = {
    "employees": ["Technician Name", "Password", "Employee Name", "Role", "Status"],
    "attendance": ["Date", "Time", "Technician Name", "Attendance Status"],
    "invoices": [
    ...
    "manual_invoices": [
        "Date", "Technician Name", "Customer Name", "Registration Number",
...
def create_excel_if_missing():
            ...
            if sheet_name not in existing:
                ...
                for col in cols:
                    if col not in df.columns:
                        if sheet_name == "employees" and col == "Technician Name" and "User ID" in df.columns:
                            df[col] = df["User ID"].astype(str)
                        else:
                            df[col] = ""
                        changed = True
    ...
    default_employees = pd.DataFrame([
        {"Technician Name": "admin", "Password": "admin123", "Employee Name": "Admin", "Role": "Admin", "Status": "Active"},
        {"Technician Name": "manager", "Password": "manager123", "Employee Name": "Manager", "Role": "Manager", "Status": "Active"},
        {"Technician Name": "mohan", "Password": "1234", "Employee Name": "Mohan", "Role": "Technician", "Status": "Active"},
        {"Technician Name": "ajay", "Password": "1234", "Employee Name": "Ajay", "Role": "Technician", "Status": "Active"},
        {"Technician Name": "vengadesh", "Password": "1234", "Employee Name": "Vengadesh", "Role": "Technician", "Status": "Active"},
        {"Technician Name": "prathisha", "Password": "1234", "Employee Name": "Prathisha", "Role": "Prathisha / System Staff", "Status": "Active"},
    ])
...
def read_sheet(sheet_name):
    ...
    if sheet_name == "employees" and "Technician Name" not in df.columns and "User ID" in df.columns:
        df["Technician Name"] = df["User ID"].astype(str)
...
def filter_entries_for_current_user(df):
    ...
    login_name = str(st.session_state.get("user_id", "")).strip().lower()
    ...
    if "User ID" in df.columns and login_name:
        return df[df["User ID"].astype(str).str.strip().str.lower() == login_name]
...
def page_login():
    ...
    with c2:
        user_id = st.text_input("Technician Name", value=st.session_state.get("remembered_user_id", ""), placeholder="Enter user id")
...
def page_dashboard():
    ...
    if is_technician():
        view_df = invoices[
            (invoices.get("Technician Name", pd.Series([""] * len(invoices))).astype(str).str.strip().str.lower() == str(st.session_state.get("employee_name", "")).strip().lower()) &
            (invoices["Date"].astype(str) == today)
        ]
        pending_req = delete_req[
            (delete_req["Technician Name"].astype(str) == user_id) &
        ...
        with c4:
            metric_card("Direct Excel", "Active", "Edit/Delete in Excel Sheet Data")
        ...
        with q3:
            quick_card("Search", "Search your service entries.", "🔍")
    ...
    if is_admin():
        ...
        with q2:
            quick_card("Excel Sheet Data", "Direct edit/delete from Excel sheet.", "📁")
        with q3:

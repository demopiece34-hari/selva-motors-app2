...
def write_sheet(sheet_name, df):
    """
    Robust direct Excel storage:
    - Updates only selected sheet.
    - Keeps all other sheets unchanged.
    - Creates workbook/sheet if missing.
    - Clears cache after save.
    """
    DATA_DIR.mkdir(exist_ok=True)

    # Keep existing workbook sheets safe
    all_sheets = {}
    if EXCEL_FILE.exists():
        try:
            with pd.ExcelFile(EXCEL_FILE, engine="openpyxl") as xls:
    ...
    # Ensure selected df has required cols
    required_cols = SHEETS.get(sheet_name, list(df.columns))
    for col in required_cols:
        if col not in df.columns:
            df[col] = ""

    # For store-only sheets, save only requested columns when possible
...
def append_row(sheet_name, row_dict):
    """
    Store row to Excel sheet immediately.
    """
    create_excel_if_missing()

    df = read_sheet(sheet_name)

    # For store-only sheets, use requested columns only.
    ...
    # For store-only sheets, save only requested columns
    if sheet_name in ["attendance", "invoices", "manual_invoices"]:

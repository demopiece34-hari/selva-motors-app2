...
def pull_single_google_sheet_to_excel(sheet_name):
    ...
    try:
        ...
        try:
            ...
            state["last_sync_time"] = datetime.now().strftime("%d-%m-%Y %I:%M:%S %p")
...
def page_cloud_excel_view_only():
    page_hero("Cloud Excel Sheet View Only", "Cloud Excel data view panna mattum. Edit/Delete options illa.", "View Only")

    st.markdown("""
    <div class="cloud-view-panel">
        <h3>☁️ Cloud Excel Sheet View Only</h3>
        <p>Indha page-la Excel sheets view panna mattum mudiyum. Edit/Delete disabled. Google Sheet edit/delete panna Pull Google Sheet → Cloud Excel use pannunga.</p>
    </div>
    """, unsafe_allow_html=True)

    if st.button("Refresh View", use_container_width=True, key="cloud_view_refresh"):
        st.rerun()

    sheet_options = list(SHEETS.keys())
    # Hide settings if you don't want sensitive settings visible? Keep view-only but still safe.
    selected_sheet = st.selectbox("Select Sheet to View", sheet_options, key="cloud_view_sheet")
    ...
    search_text = st.text_input("Search in selected sheet", key="cloud_view_search")
    view_df = df.copy()
    ...
    st.caption("View-only mode. Data edit/delete options removed.")
...
def menu_page():
    ...
    if is_admin():
        pages = [
            ...
            "Customer Service History", "Cloud Excel View Only", "Admin Panel"
        ]
    elif is_manager():
        pages = [
...
def page_duplicate_upload_finder():
    page_hero("Duplicate Upload Finder", "Manager can view duplicate Job Card uploads and direct delete selected duplicate row.", "Manager")
    ...
    display_cols = ["Original Row Number"] + show_cols
    st.dataframe(dup_rows[display_cols], use_container_width=True)
    ...
    if selected_index in invoices.index:
...
def main():
    ...
    elif page == "Cloud Excel View Only":
        if is_admin():
            page_cloud_excel_view_only()
        else:

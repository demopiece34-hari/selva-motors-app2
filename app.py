    ...
    try:
        ...
        if client is not None and sheet_id:
            ...
            for sheet_name, cols in SHEETS.items():
                ws = get_or_create_google_worksheet_primary(spreadsheet, sheet_name, 100, max(len(cols) + 5, 20))
    ...
    if df is not None:
        ...
        for col in SHEETS.get(sheet_name, []):
            if col not in df.columns:
                df[col] = ""
        return df[SHEETS.get(sheet_name, list(df.columns))].fillna("")
...
def write_sheet(sheet_name, df):
    ...
    hero_loading_overlay("Updating Google Sheet", "Please wait")
...
def append_row(sheet_name, row_dict):
    ...
    hero_loading_overlay("Saving to Google Sheet", "Please wait")
...
def require_hero_show(page_key, button_text, loading_text="Loading..."):
    ...
    if not st.session_state.get(session_key, False):
        ...
        if st.button(button_text, use_container_width=True, key=f"{page_key}_hero_show_button"):
            ...
            st.rerun()
        return False
    hero_loading_overlay(loading_text, "Please wait")
    return True

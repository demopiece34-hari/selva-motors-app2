    ...
    if not EXCEL_FILE.exists():
        with pd.ExcelWriter(EXCEL_FILE, engine="openpyxl") as writer:
            for sheet, cols in SHEETS.items():
                ...
                if sheet == "settings":
                    version = VERSION_TEXT if "VERSION_TEXT" in globals() else "Direct Excel Sheet Storage Version"
                    df = pd.DataFrame([
                        ["Storage Type", "Direct Excel Sheet"],
                        ["Version", version],
...
def write_sheet(sheet_name, df):
    ...
    # Direct mode:
    # Save directly to local Excel sheet file. No cloud sync required.
    try:

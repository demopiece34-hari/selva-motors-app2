...
/* ============================================================
   SIMPLE IMPROVEMENT PATCH - PULL TIME + LOCATION ON BUTTON
   ============================================================ */
.location-on-card {
...
.location-on-btn {
    ...
    font-weight: 900;
    text-decoration: none !important;
    box-shadow: 0 14px 32px rgba(22,163,74,.20);
}
.pull-time-card {
    background: linear-gradient(135deg, #ffffff, #eff6ff);
    border: 1px solid rgba(37,99,235,.18);
...
.pull-time-card b { color: #0f172a; }
.pull-time-card span { color: #2563eb; font-weight: 900; }
...
def pull_single_google_sheet_to_excel(sheet_name):
    ...
    try:
        ...
        set_last_google_pull_time(sheet_name)
...
def location_on_help_card():
    st.markdown("""
    <div class="location-on-card">
        <h3>📍 Turn ON Location / Allow GPS</h3>
        <p>
            Attendance mark panna phone/browser location permission ON irukkanum.
            Button click pannitu browser popup-la <b>Allow</b> select pannunga.
        </p>
        <a class="location-on-btn" href="javascript:location.reload()">📍 Turn ON Location / Refresh GPS</a>
    </div>
    """, unsafe_allow_html=True)


def set_last_google_pull_time(sheet_name=""):
    try:
        state = load_google_sync_state()
        state["last_pull_time"] = datetime.now().strftime("%d-%m-%Y %I:%M:%S %p")
        state["last_pull_sheet"] = sheet_name
        if "save_google_sync_state" in globals():
...
def page_admin_panel():
    ...
    with tabs[3]:
        ...
        pull_state = load_google_sync_state()
        last_pull_time = pull_state.get("last_pull_time", "Not yet")
        last_pull_sheet = pull_state.get("last_pull_sheet", "-")
        st.markdown(f"""
        <div class="pull-time-card">
            <b>Last Google Pull:</b> <span>{last_pull_time}</span><br>
            <b>Last Pulled Sheet:</b> <span>{last_pull_sheet}</span>
        </div>
        """, unsafe_allow_html=True)

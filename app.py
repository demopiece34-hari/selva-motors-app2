...
/* ===== SELVA MOTORS V5 FAST OPTIMAL UI ===== */
.v5-panel{background:linear-gradient(135deg,#fff,#fff7f7);border:1px solid rgba(227,24,55,.13);border-radius:22px;padding:16px;box-shadow:0 14px 34px rgba(17,24,39,.07);margin:10px 0 16px 0}.v5-panel h3{margin:0 0 6px 0;font-size:18px;font-weight:900;color:#111827}.v5-panel p{margin:0;color:#64748b;font-size:13px;font-weight:700}.v5-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin:12px 0 18px 0}.v5-card{background:rgba(255,255,255,.94);border:1px solid rgba(226,232,240,.95);border-radius:20px;padding:14px;box-shadow:0 12px 30px rgba(17,24,39,.07)}.v5-card small{display:block;color:#64748b;font-size:11px;font-weight:900;text-transform:uppercase}.v5-card b{display:block;color:#111827;font-size:22px;font-weight:900;margin-top:5px}.v5-card span{color:#e31837;font-size:12px;font-weight:800}.v5-alert{border-radius:18px;padding:12px 14px;background:#fff7ed;border:1px solid #fed7aa;color:#7c2d12;font-weight:800;margin:8px 0 12px 0}.v5-success{border-radius:18px;padding:12px 14px;background:#ecfdf5;border:1px solid #bbf7d0;color:#166534;font-weight:800;margin:8px 0 12px 0}@media(max-width:900px){.v5-grid{grid-template-columns:repeat(1,minmax(0,1fr))}}
...
SHEETS = {
    ...
    "manual_invoices": [
        ...
        "Payment Mode", "Remarks", "Total Amount", "PDF File", "Status"
    ],
    "settings": [
        "Key", "Value"
    ],
    "audit_log": [
        "Log ID", "Date", "Time", "User ID", "Employee Name", "Role",
        "Action", "Sheet Name", "Reference ID", "Old Value", "New Value"
    ],
    "app_notifications": [
        "Notification ID", "Date", "Time", "Role", "User ID",
...
def generate_manual_bill_pdf(customer_name, reg_no, bike_model, spare_rows, labour_amount, service_type, payment_mode="Cash", remarks=""):
...
def page_login():
    ...
    with c2:
        ...
        failed_count = st.session_state.get("login_failed_count", 0)
        if failed_count >= 3:
            st.warning("3 wrong attempts detected. Correct password enter pannunga; successful login reset aagum.")
        ...
        if st.button("Login", use_container_width=True):
            ...
            if user:
                ...
                st.session_state["login_failed_count"] = st.session_state.get("login_failed_count", 0) + 1
...
def v5_panel(title, body):
    st.markdown(f"""
    <div class="v5-panel"><h3>{title}</h3><p>{body}</p></div>
    """, unsafe_allow_html=True)


def v5_cards(items):
    html = '<div class="v5-grid">'
    for label, value, note in items:
        html += '<div class="v5-card">' + f'<small>{label}</small><b>{value}</b><span>{note}</span>' + '</div>'
    html += '</div>'
    st.markdown(html, unsafe_allow_html=True)


def add_audit_log(action, sheet_name="", reference_id="", old_value="", new_value=""):
    try:
        append_row("audit_log", {
            "Log ID": "LOG-" + uuid.uuid4().hex[:8].upper(),
            "Date": today_str(), "Time": time_str(),
            "User ID": st.session_state.get("user_id", ""),
            "Employee Name": st.session_state.get("employee_name", ""),
            "Role": st.session_state.get("role", ""),
            "Action": action, "Sheet Name": sheet_name,
            "Reference ID": reference_id,
            "Old Value": str(old_value)[:500], "New Value": str(new_value)[:500]
        })
    except Exception:
        pass


def add_notification(role, user_id, title, message):
    try:
        append_row("app_notifications", {
            "Notification ID": "N-" + uuid.uuid4().hex[:8].upper(),
            "Date": today_str(), "Time": time_str(),
            "Role": role, "User ID": user_id,
...
def show_v5_notifications():
    try:
        df = read_sheet("app_notifications")
        if df.empty:
            return
        user_role = st.session_state.get("role", "")
        user_id = st.session_state.get("user_id", "")
        notes = df[(df["Status"].astype(str) == "Unread") & ((df["Role"].astype(str) == user_role) | (df["Role"].astype(str) == "All") | (df["User ID"].astype(str) == user_id))]
        if not notes.empty:
            st.markdown(f"<div class='v5-alert'>🔔 {len(notes)} notification(s) pending</div>", unsafe_allow_html=True)
            with st.expander("View Notifications"):
                st.dataframe(notes[["Date", "Time", "Title", "Message"]], use_container_width=True)
    except Exception:
        pass


def invoice_validation_warnings(data):
    warnings = []
    spare = to_float(data.get("Spare Amount", 0))
    labour = to_float(data.get("Labour Amount", 0))
    total = to_float(data.get("Total Amount", 0))
    if total > 0 and (spare + labour) > 0 and abs((spare + labour) - total) > 2:
        warnings.append(f"Spare + Labour total mismatch: ₹{spare + labour:.2f} vs Invoice Total ₹{total:.2f}")
    for col in ["Job Card Number", "Registration Number", "Bike Model"]:
        if not str(data.get(col, "")).strip():
            warnings.append(col + " missing")
    return warnings


def generate_customer_history_pdf(df, reg_no):
    pdf_path = PDF_DIR / f"customer_history_{clean_reg_no(reg_no) or 'all'}.pdf"
    c = canvas.Canvas(str(pdf_path), pagesize=A4); w, h = A4
    try: draw_pdf_header(c, w, h, "Customer Service History", "Vehicle service timeline", f"Reg No: {clean_reg_no(reg_no)}", f"Date: {today_str()}")
    except Exception:
        c.setFont("Helvetica-Bold", 16); c.drawString(38, h-45, "SELVA MOTORS - Customer Service History")
    y = h - 140
    total_visits = len(df)
    total_amount = pd.to_numeric(df.get("Total Amount", pd.Series([])), errors="coerce").fillna(0).sum() if not df.empty else 0
    try:
        pdf_label_value_box(c, 38, y, 160, "Total Visits", total_visits, "#F8FAFC")
        pdf_label_value_box(c, 218, y, 160, "Total Spent", f"Rs.{total_amount:.2f}", "#FFF1F2")
        pdf_label_value_box(c, 398, y, 155, "Reg Number", clean_reg_no(reg_no), "#F8FAFC")
    except Exception: pass
    y -= 70
    headers=["S.No","Date","Technician","Service","Bike Model","Labour","Total"]; widths=[32,70,82,80,105,60,60]
    xs=[38]
    for ww in widths[:-1]: xs.append(xs[-1]+ww)
    def head(yy):
        c.setFillColor(colors.HexColor("#111827")); c.roundRect(38, yy-6, sum(widths), 22, 7, fill=True, stroke=False)
        c.setFillColor(colors.white); c.setFont("Helvetica-Bold",7)
        for i,hx in enumerate(headers): c.drawString(xs[i]+3, yy, hx)
    head(y); y-=24; c.setFont("Helvetica",7); c.setFillColor(colors.black)
    if df.empty: c.drawString(38,y,"No service history found.")
    else:
        for idx,row in df.reset_index(drop=True).iterrows():
            if y < 70:
                try: draw_pdf_footer(c,w)
                except Exception: pass
                c.showPage(); y=h-130; head(y); y-=24; c.setFont("Helvetica",7)
            vals=[str(idx+1),str(row.get("Date",""))[:10],str(row.get("Technician Name",""))[:14],str(row.get("Service Type",""))[:14],str(row.get("Bike Model",""))[:18],f"Rs.{to_float(row.get('Labour Amount',0)):.0f}",f"Rs.{to_float(row.get('Total Amount',0)):.0f}"]
            if idx%2==0:
                c.setFillColor(colors.HexColor("#F8FAFC")); c.roundRect(38,y-6,sum(widths),17,4,fill=True,stroke=False); c.setFillColor(colors.black)
            for i,v in enumerate(vals): c.drawString(xs[i]+3,y,v)
            y-=17
    try: draw_pdf_footer(c,w)
    except Exception: pass
    c.save(); return str(pdf_path)


def generate_technician_performance_pdf(df, att_df, tech_name="All Technicians"):
    pdf_path = PDF_DIR / f"technician_performance_{re.sub(r'[^A-Za-z0-9_-]', '_', str(tech_name))}.pdf"
    c = canvas.Canvas(str(pdf_path), pagesize=A4); w,h=A4
    try: draw_pdf_header(c,w,h,"Technician Performance","Service + Attendance Summary",f"Technician: {tech_name}",f"Date: {today_str()}")
    except Exception:
        c.setFont("Helvetica-Bold",16); c.drawString(38,h-45,"SELVA MOTORS - Technician Performance")
    y=h-140
    entries=len(df); labour=pd.to_numeric(df.get("Labour Amount", pd.Series([])), errors="coerce").fillna(0).sum() if not df.empty else 0
    present=len(att_df[att_df["Attendance Status"].astype(str).str.lower()=="present"]) if not att_df.empty and "Attendance Status" in att_df.columns else 0
    try:
        pdf_label_value_box(c,38,y,150,"Entries",entries,"#F8FAFC"); pdf_label_value_box(c,208,y,170,"Labour",f"Rs.{labour:.2f}","#FFF1F2"); pdf_label_value_box(c,398,y,150,"Present Days",present,"#F8FAFC")
    except Exception: pass
    y-=75; c.setFillColor(colors.HexColor("#0F172A")); c.setFont("Helvetica-Bold",12); c.drawString(38,y,"Technician-wise Summary"); y-=22
    if df.empty:
        c.setFont("Helvetica",9); c.drawString(38,y,"No service records found.")
    else:
        summary=df.groupby("Technician Name", dropna=False).agg(Entries=("Entry ID","count"), Labour=("Labour Amount","sum"), Revenue=("Total Amount","sum")).reset_index()
        xs=[38,210,310,420]; heads=["Technician","Entries","Labour","Revenue"]
        c.setFillColor(colors.HexColor("#111827")); c.roundRect(38,y-6,500,22,7,fill=True,stroke=False); c.setFillColor(colors.white); c.setFont("Helvetica-Bold",8)
        for i,hx in enumerate(heads): c.drawString(xs[i],y,hx)
        y-=24; c.setFillColor(colors.black); c.setFont("Helvetica",8)
        for _,row in summary.iterrows():
            c.drawString(xs[0],y,str(row.get("Technician Name",""))[:26]); c.drawString(xs[1],y,str(row.get("Entries",0))); c.drawString(xs[2],y,f"Rs.{float(row.get('Labour',0)):.0f}"); c.drawString(xs[3],y,f"Rs.{float(row.get('Revenue',0)):.0f}"); y-=18
    try: draw_pdf_footer(c,w)
    except Exception: pass
    c.save(); return str(pdf_path)
...
def page_dashboard():
    ...
    v5_panel("V5 Production Summary", "Fast report, attendance, customer history, audit log and alert system enabled.")
    show_v5_notifications()
...
def page_upload_invoice():
    ...
    validation_warnings = invoice_validation_warnings(data)
    if validation_warnings:
        st.markdown("<div class='v5-alert'>⚠️ " + "<br>".join(validation_warnings) + "</div>", unsafe_allow_html=True)
    ...
    if st.button("Click to Proceed the Entry", use_container_width=True):
        ...
        with st.spinner("Please wait... Entry processing. Do not upload another file."):
            ...
            if final_duplicate:
                ...
                entry_id = save_invoice_entry_from_data(data, entry_type="OCR Upload")
                add_audit_log("Invoice Saved", "invoices", entry_id, "", data)
                processing_wait_3s("Please wait, Excel entry processing")
                st.session_state.pop("ocr_preview", None)
...
def page_reports():
    ...
    tabs = st.tabs(["📑 Service Reports", "🏍️ Daily Technician Report", "📍 Attendance Report", "📈 Performance Report"])
    ...
    with tabs[2]:
        report_control_panel("Attendance Report", "All users or particular employee select pannitu attendance report view/download panna mudiyum.")
        ...
        if attendance_df.empty:
            ...
            if is_technician() or is_prathisha():
                ...
                st.info("User view: your own attendance only.")
                att_view_type = "My Attendance"
                selected_att_user = st.session_state.get("employee_name", "")
            else:
                att_view_type = st.radio("Attendance Report View Type", ["All Users", "Particular User"], horizontal=True, key="attendance_report_view_type")
                ...
                if att_view_type == "Particular User":
                    ...
                    selected_att_user = st.selectbox("Select User", users, key="attendance_report_user")
                    att_df = att_df[att_df["Technician Name"].astype(str) == selected_att_user]

                c1, c2 = st.columns(2)
                with c1:
                    att_date_filter = st.text_input("Attendance Date Filter DD-MM-YYYY", value="", key="attendance_report_date")
                with c2:
                    att_status_filter = st.selectbox("Attendance Status", ["All", "Present", "Absent", "Half Day"], key="attendance_report_status")

                if att_date_filter.strip():
                    att_df = att_df[att_df["Date"].astype(str) == att_date_filter.strip()]
            ...
            report_summary_cards([
                ("Attendance Rows", len(att_df), "Selected records"),
                ("Present Count", present_count, "GPS marked"),
                ("Selected User", selected_att_user, "Attendance view"),
                ("Report Type", att_view_type, "View mode"),
            ])
            ...
            if st.button("Generate Attendance PDF Report", use_container_width=True, key="generate_attendance_pdf_report"):
                att_pdf = generate_attendance_report_pdf(att_df, f"{att_view_type} - {selected_att_user}", "selva_motors_attendance_report.pdf")
                st.session_state["attendance_report_pdf"] = att_pdf
                st.success("Attendance PDF report generated.")

            if st.session_state.get("attendance_report_pdf"):
                att_pdf_path = st.session_state["attendance_report_pdf"]
                if Path(att_pdf_path).exists():
                    with open(att_pdf_path, "rb") as f:
                        st.download_button("Download Attendance PDF Report", f, file_name=Path(att_pdf_path).name, mime="application/pdf", use_container_width=True, key="download_attendance_pdf_report")

    with tabs[3]:
        report_control_panel("Technician Performance Report", "Technician-wise entries, labour amount, revenue and attendance summary.")
        perf_df = invoices.copy()
        perf_att = attendance_df.copy()
        tech_options = ["All Technicians"]
        if not perf_df.empty and "Technician Name" in perf_df.columns:
            tech_options += sorted([x for x in perf_df["Technician Name"].astype(str).unique().tolist() if x.strip()])
        selected_perf_tech = st.selectbox("Select Technician", tech_options, key="perf_tech")
        perf_month = st.text_input("Month Filter MM-YYYY Optional", value="", key="perf_month")
        if selected_perf_tech != "All Technicians":
            perf_df = perf_df[perf_df["Technician Name"].astype(str) == selected_perf_tech]
            if not perf_att.empty and "Technician Name" in perf_att.columns:
                perf_att = perf_att[perf_att["Technician Name"].astype(str) == selected_perf_tech]
        if perf_month and not perf_df.empty:
            temp_dates = pd.to_datetime(perf_df["Date"], format="%d-%m-%Y", errors="coerce")
            perf_df = perf_df[temp_dates.dt.strftime("%m-%Y") == perf_month]
        if perf_month and not perf_att.empty and "Date" in perf_att.columns:
            att_dates = pd.to_datetime(perf_att["Date"], format="%d-%m-%Y", errors="coerce")
            perf_att = perf_att[att_dates.dt.strftime("%m-%Y") == perf_month]
        perf_entries = len(perf_df)
        perf_labour = pd.to_numeric(perf_df.get("Labour Amount", pd.Series([])), errors="coerce").fillna(0).sum() if not perf_df.empty else 0
        perf_revenue = pd.to_numeric(perf_df.get("Total Amount", pd.Series([])), errors="coerce").fillna(0).sum() if not perf_df.empty else 0
        perf_present = len(perf_att[perf_att["Attendance Status"].astype(str).str.lower() == "present"]) if not perf_att.empty and "Attendance Status" in perf_att.columns else 0
        report_summary_cards([
            ("Entries", perf_entries, "Service work"),
            ("Labour", f"₹{perf_labour:,.0f}", "Performance amount"),
            ("Revenue", f"₹{perf_revenue:,.0f}", "Total service"),
            ("Present Days", perf_present, "Attendance"),
        ])
        cols=[c for c in ["Date","Technician Name","Registration Number","Bike Model","Service Type","Labour Amount","Total Amount"] if c in perf_df.columns]
        if cols:
            st.dataframe(perf_df[cols], use_container_width=True)
        if st.button("Generate Technician Performance PDF", use_container_width=True, key="perf_pdf"):
            pdf = generate_technician_performance_pdf(perf_df, perf_att, selected_perf_tech)
            st.session_state["performance_pdf"] = pdf
            st.success("Technician Performance PDF generated.")
        if st.session_state.get("performance_pdf"):
            pdf_path = st.session_state["performance_pdf"]
            if Path(pdf_path).exists():
                with open(pdf_path, "rb") as f:
                    st.download_button("Download Technician Performance PDF", f, file_name=Path(pdf_path).name, mime="application/pdf", use_container_width=True)
...
def page_customer_service_history():
    ...
    if reg:
        ...
        total_spent = pd.to_numeric(result.get("Total Amount", pd.Series([])), errors="coerce").fillna(0).sum() if not result.empty else 0
        last_service = result["Date"].astype(str).iloc[-1] if not result.empty and "Date" in result.columns else "-"
        v5_cards([
            ("Total Visits", total_visits, "Customer vehicle history"),
            ("Total Spent", f"₹{total_spent:,.0f}", "All service entries"),
            ("Last Service", last_service, "Latest record"),
            ("Oil Status", "Check Rows", "Oil Yes/No visible"),
        ])
        st.dataframe(result, use_container_width=True)
        if st.button("Generate Customer History PDF", use_container_width=True):
            pdf = generate_customer_history_pdf(result, reg_clean)
            st.session_state["customer_history_pdf"] = pdf
            st.success("Customer History PDF generated.")
        if st.session_state.get("customer_history_pdf"):
            pdf_path = st.session_state["customer_history_pdf"]
            if Path(pdf_path).exists():
                with open(pdf_path, "rb") as f:
                    st.download_button("Download Customer History PDF", f, file_name=Path(pdf_path).name, mime="application/pdf", use_container_width=True)
...
def page_manual_invoice():
    ...
    payment_mode = c_pay1.selectbox("Payment Mode", ["Cash", "UPI", "Card", "Credit", "Other"])
    remarks = c_pay2.text_input("Remarks", placeholder="Optional remarks")
    ...
    if st.button("Generate Manual Bill PDF", use_container_width=True):
        ...
        pdf = generate_manual_bill_pdf(customer_name, reg_no, bike_model, spare_rows, labour_amount, service_type, payment_mode, remarks)
...
def page_delete_invoice_request():
    ...
    if st.button("Submit Delete Request"):
        ...
        add_audit_log("Delete Request Sent", "delete_requests", entry_id, "", reason)
        add_notification("Admin", "", "Delete Request Pending", f"{st.session_state.get('employee_name', '')} requested delete for {entry_id}")
...
def admin_excel_data_manager():
    ...
    if action == "Add Row":
        ...
        if st.button("Add Row to Excel", use_container_width=True, key=f"add_btn_{sheet_name}"):
            ...
            add_audit_log("Add Row", sheet_name, "", "", new_row)
            st.cache_data.clear()
        ...
        if st.button("Save Edited Row", use_container_width=True, key=f"save_edit_{sheet_name}"):
            ...
            if selected_idx >= len(fresh_df):
                ...
                return
            # Pandas 2.x / Python 3.14 safe edit:
            # convert to object to avoid TypeError when editing numeric columns with text input.
            fresh_df = fresh_df.astype("object")
            for col in SHEETS[sheet_name]:
                fresh_df.at[selected_idx, col] = str(edited_row.get(col, ""))
            write_sheet(sheet_name, fresh_df)
            add_audit_log("Edit Row", sheet_name, str(selected_idx + 1), "", edited_row)
            st.cache_data.clear()
        ...
        if st.button("Delete Selected Row", use_container_width=True, key=f"delete_btn_{sheet_name}"):
            ...
            old_deleted_row = fresh_df.iloc[selected_idx].to_dict()
            fresh_df = fresh_df.drop(index=selected_idx).reset_index(drop=True)
            write_sheet(sheet_name, fresh_df)
            add_audit_log("Delete Row", sheet_name, str(selected_idx + 1), old_deleted_row, "")
            st.cache_data.clear()
...
def page_admin_panel():
    ...
    with tabs[1]:
        st.markdown("<div class='admin-tab-note'>Employee V5 Control: Add/update user, role, status and password reset pannalam.</div>", unsafe_allow_html=True)
    ...
    with tabs[4]:
        ...
        if c_manual1.button("Retry / Manual Sync Changed Sheets Now", use_container_width=True):
    ...
    with tabs[5]:
        ...
        st.subheader("Create App Notification")
        n_role = st.selectbox("Notification Role", ["All", "Admin", "Manager", "Technician", "Prathisha / System Staff"], key="notify_role")
        n_title = st.text_input("Notification Title", key="notify_title")
        n_msg = st.text_area("Notification Message", key="notify_msg")
        if st.button("Send Notification", use_container_width=True):
            if n_title and n_msg:
                add_notification(n_role, "", n_title, n_msg)
                add_audit_log("Notification Sent", "app_notifications", n_role, "", n_title)
                st.success("Notification sent.")
            else:
                st.warning("Title and message required.")
...
def page_duplicate_upload_finder():
    ...
    st.markdown("""
    <div class="duplicate-compare-box">
        <b>Duplicate Finder V4</b><br>
        Same full Job Card Number mattum duplicate-aa consider pannum. Select row compare pannitu delete pannunga.
    </div>
    """, unsafe_allow_html=True)
    ...
    with tabs[6]:
        st.markdown("<div class='admin-tab-note'>Audit Log: edit/delete/add/delete request actions track pannum.</div>", unsafe_allow_html=True)
        ...
        if audit.empty:
            ...
            c1, c2 = st.columns(2)
            with c1:
                audit_search = st.text_input("Search Audit", key="audit_search")
            with c2:
                audit_date = st.text_input("Audit Date DD-MM-YYYY", key="audit_date")
            show_audit = audit.copy()
            if audit_search:
                show_audit = show_audit[show_audit.astype(str).apply(lambda row: row.str.contains(audit_search, case=False, na=False).any(), axis=1)]
            if audit_date:
                show_audit = show_audit[show_audit["Date"].astype(str) == audit_date]
            st.dataframe(show_audit.tail(300), use_container_width=True)
...
def page_manager_edit():
    ...
    if st.button("Update Status"):
        ...
        if len(idx) > 0:
            ...
            add_audit_log("Manager Status Update", "invoices", entry_id, "", new_status)
...
def run_app_safely():
    try:
        ...
        st.error("App-la small error vandhurukku. Please refresh pannunga or Admin-kku inform pannunga.")
        with st.expander("Technical Error Details"):
            st.code(str(e))
        try:
            add_audit_log("App Error", "runtime", "", "", str(e))
        except Exception:
            pass

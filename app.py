...
st.set_page_config(
    page_title="Selva Motors Google Sheet Only App",
    page_icon="🏍️",
    layout="wide"
)

APP_TITLE = "SELVA MOTORS EXCEL STORAGE APP"
VERSION_TEXT = "Google Sheet Only Version"
...
def create_excel_if_missing():
    """Google Sheet only storage bootstrap. No local Excel file is created."""
    DATA_DIR.mkdir(exist_ok=True)
    UPLOAD_DIR.mkdir(exist_ok=True)
    PDF_DIR.mkdir(exist_ok=True)
    BACKUP_DIR.mkdir(exist_ok=True)
    ...
    try:
        ...
        if client is not None and sheet_id:
            spreadsheet = client.open_by_key(sheet_id)
            for sheet_name, cols in SHEETS.items():
                ws = get_or_create_worksheet(spreadsheet, sheet_name, rows=100, cols=max(len(cols) + 5, 20))
                values = ws.get_all_values()
                if not values and cols:
...
# Google Sheet is primary storage. No local Excel backup.
# ============================================================
...
def get_or_create_google_worksheet_primary(spreadsheet, sheet_name, rows=1000, cols=30):
    return get_or_create_worksheet(spreadsheet, sheet_name, rows=rows, cols=cols)
...
def google_read_sheet(sheet_name):
    try:
        client, err = google_sheet_client()
        if client is None:
            return pd.DataFrame(columns=SHEETS.get(sheet_name, [])), err

        sheet_id = get_google_sheet_id()
        if not sheet_id:
            return pd.DataFrame(columns=SHEETS.get(sheet_name, [])), "SHEET_ID missing in secrets"

        spreadsheet = client.open_by_key(sheet_id)
        try:
            ws = spreadsheet.worksheet(sheet_name)
        except Exception:
            ws = spreadsheet.add_worksheet(title=sheet_name, rows=100, cols=max(len(SHEETS.get(sheet_name, [])) + 5, 20))
            header = SHEETS.get(sheet_name, [])
            if header:
        ...
        return pd.DataFrame(columns=SHEETS.get(sheet_name, [])), str(e)
...
def google_write_sheet(sheet_name, df, allow_delete_update=True):
    try:
        ...
        sheet_id = get_google_sheet_id()
        if not sheet_id:
            return False, "SHEET_ID missing in secrets"
        ...
        try:
            ...
            ws = spreadsheet.add_worksheet(title=sheet_name, rows=max(len(df) + 20, 100), cols=max(len(cols) + 5, 20))

        ws.clear()
        clean_df = df.copy().fillna("")
        clean_df = clean_df.astype(str)
        clean_df = clean_df[[c for c in cols if c in clean_df.columns]] if cols else clean_df
        values = [clean_df.columns.tolist()] + clean_df.values.tolist()
        if values:
            ws.update(values, value_input_option="USER_ENTERED")
        return True, f"{sheet_name} saved to Google Sheet"
    except Exception as e:
        return False, str(e)
...
def google_append_row(sheet_name, row_dict):
    try:
        current, _ = google_read_sheet(sheet_name)
        cols = SHEETS.get(sheet_name, list(row_dict.keys()))
        if current.empty and not cols:
            cols = list(row_dict.keys())
        new_row = {c: row_dict.get(c, "") for c in cols}
        new_df = pd.concat([current, pd.DataFrame([new_row])], ignore_index=True)
        return google_write_sheet(sheet_name, new_df)
    except Exception as e:
        return False, str(e)
...
def is_google_auto_sync_enabled():
    ...
    Google Sheet remains primary storage.
    """
...
def sync_single_excel_sheet_to_google_sheet(sheet_name):
    return False, "Local Excel / cloud sync removed. Google Sheet is primary storage only."
...
def load_google_sync_state():
    return {
        ...
        "last_sync_message": "Google Sheet only",
...
def sync_changed_sheets_to_google():
    return True, "Google Sheet only storage active"
...
def login_user(user_id, password):
    ...
    required = ["User ID", "Password", "Employee Name", "Role", "Status"]
    for col in required:
...
def processing_wait_3s(message="Processing entry"):
    box = st.empty()
    box.info(f"{message}... Please wait")
    return box



def require_hero_show(page_key, button_text, loading_text="Loading..."):
    session_key = f"hero_show_unlocked_{page_key}"
...
def hero_loading_overlay(message="Loading...", wait_text="Please wait"):
    st.markdown(f"""
    <style>
    ...
    .hero-wait-card {{
        ...
        box-shadow:0 32px 95px rgba(127,29,29,.28);
        text-align:center;
    ...
    .hero-wait-logo {{
        ...
        letter-spacing:3px;
        margin-bottom:18px;
        animation:heroPulse 1.1s infinite ease-in-out;
    }}
    .hero-wait-spinner {{
        width:72px;
        height:72px;
    ...
    <div class="hero-fullscreen-wait">
      <div class="hero-wait-card">
        <div class="hero-wait-logo">HERO</div>
        <div class="hero-wait-spinner"></div>
        <div class="hero-wait-title">{message}</div>
        <div class="hero-wait-sub">{wait_text}</div>
...
def hero_waiting_loader(message="Saving to Google Sheet", seconds=0):
    hero_loading_overlay(message, "Please wait")
        ...
        <div class="login-hero">
            <div class="hero-login-panel">
                ...
                <div class="feature-strip" style="justify-content:center;margin-bottom:18px;">
                    <div class="feature-pill">Google Sheet Only</div>
...
def get_dashboard_summary_cache():
    invoices = read_sheet("invoices")
    attendance = read_sheet("attendance")
    today = today_str()
    today_inv = invoices.copy()
    if not today_inv.empty and "Date" in today_inv.columns:
        today_inv = today_inv[today_inv["Date"].astype(str) == today]
    if "Total Amount" in today_inv.columns:
        total_amt = pd.to_numeric(today_inv["Total Amount"], errors="coerce").fillna(0).sum()
    else:
        total_amt = 0
    if "Labour Amount" in today_inv.columns:
        labour_amt = pd.to_numeric(today_inv["Labour Amount"], errors="coerce").fillna(0).sum()
    else:
        labour_amt = 0
    return {
        "today_rows": int(len(today_inv)),
        "today_total_amount": float(total_amt),
        "today_labour_amount": float(labour_amt),
        "attendance_rows": int(len(attendance)),
    }
...
# ============================================================
def page_attendance():
    if not require_hero_show("attendance", "📍 SHOW ATTENDANCE", "Loading Attendance..."):
        return
...
# ============================================================
def page_search():
    if not require_hero_show("search", "🔍 SHOW SEARCH", "Loading Search..."):
        return
...
def page_customer_service_history():
    if not require_hero_show("customer_service_history", "📖 SHOW HISTORY", "Loading History..."):
        return
...
def render_cloud_excel_view_only():
    st.markdown("""
    <div class="cloud-view-panel">
        <h3>☁️ Google Sheet View Only</h3>
        <p>Admin Panel-kulla view only. Edit/Delete options illa. Google Sheet edit/delete panna Direct Google Sheet use pannunga.</p>
    </div>
    """, unsafe_allow_html=True)
...
def page_admin_panel():
    if not require_hero_show("admin_panel", "⚙️ SHOW ADMIN PANEL", "Loading Admin Panel..."):
        return
    page_hero("Admin Panel", "Google Sheet control center: revenue, employees, delete requests, data manager and settings.", "Admin")
    ...
    with tabs[3]:
        ...
        st.caption("Manual sync removed. Google Sheet edit/delete panna Direct Google Sheet use pannunga.")
...
def page_duplicate_upload_finder():
    if not require_hero_show("duplicate_upload_finder", "🔎 SHOW DUPLICATES", "Loading Duplicate Finder..."):
        return
...
# ============================================================
# FAST GOOGLE SHEET ONLY OVERRIDES (final runtime definitions)
# ============================================================
    ...
    try:
        ...
        try:
            ...
            if SHEETS.get(sheet_name):
                ws.update([SHEETS[sheet_name]], value_input_option="USER_ENTERED")
...
def google_write_sheet(sheet_name, df, allow_delete_update=True):
    try:
        ...
        if cols:
            ordered = [c for c in cols if c in clean_df.columns]
            if ordered:
                clean_df = clean_df[ordered]
        values = [clean_df.columns.tolist()] + clean_df.values.tolist()
        if values:
            ws.update(values, value_input_option="USER_ENTERED")
        return True, f"{sheet_name} saved to Google Sheet"
...
def create_excel_if_missing():
    """Google Sheet only bootstrap. No local Excel file is created."""
    DATA_DIR.mkdir(exist_ok=True)
    UPLOAD_DIR.mkdir(exist_ok=True)
    PDF_DIR.mkdir(exist_ok=True)
    BACKUP_DIR.mkdir(exist_ok=True)
    try:
        client, _ = google_sheet_client()
        sheet_id = get_google_sheet_id()
        if client is not None and sheet_id:
            spreadsheet = client.open_by_key(sheet_id)
            for sheet_name, cols in SHEETS.items():
                ws = get_or_create_worksheet(spreadsheet, sheet_name, rows=100, cols=max(len(cols) + 5, 20))
                if not ws.get_all_values() and cols:
                    ws.update([cols], value_input_option="USER_ENTERED")
    except Exception:
        pass
...
def clean_duplicate_columns(df):
    """Remove duplicate column names safely and preserve first occurrence."""
    if df is None:
        return df
    try:
...
def read_sheet(sheet_name):
    create_excel_if_missing()
    df, _ = google_read_sheet(sheet_name)
    if df is None:
        return pd.DataFrame(columns=SHEETS.get(sheet_name, []))
    df = clean_duplicate_columns(df)
    for col in SHEETS.get(sheet_name, []):
...
def page_cloud_excel_view_only():
    st.info("Google Sheet only storage active. This view has been removed.")


def page_admin_panel():
    if not require_hero_show("admin_panel", "⚙️ SHOW ADMIN PANEL", "Loading Admin Panel..."):
        return
    page_hero("Admin Panel", "Google Sheet control center: revenue, employees, delete requests and settings.", "Admin")
    premium_panel("Admin Control Center", "Clean admin access: reports, customer history, employee control and settings.")
    ...
    tabs = st.tabs(["📊 Revenue", "👥 Employees", "🗑️ Delete Requests", "⚙️ Settings"])

    with tabs[0]:
        st.markdown("<div class='admin-tab-note'>Admin revenue overview.</div>", unsafe_allow_html=True)
        temp = invoices.copy()
        temp["Month"] = pd.to_datetime(temp.get("Date", pd.Series(dtype=str)), format="%d-%m-%Y", errors="coerce").dt.strftime("%m-%Y")
        month_key = app_now().strftime("%m-%Y")
        month_df = temp[temp["Month"] == month_key] if "Month" in temp.columns else invoices
        c1, c2, c3 = st.columns(3)
        with c1:
            ultra_card("Monthly Revenue", f"₹{month_df['Total Amount'].sum():,.0f}", "Admin only", "💰")
        with c2:
            ultra_card("Invoice Rows", len(invoices), "Stored invoices", "🏍️")
        with c3:
            ultra_card("Total Revenue", f"₹{invoices['Total Amount'].sum():,.0f}", "All entries", "📈")

    with tabs[1]:
        st.markdown("<div class='admin-tab-note'>Employee login data is stored directly in Google Sheet.</div>", unsafe_allow_html=True)
        employees = read_sheet("employees")
        st.dataframe(employees, use_container_width=True)

    with tabs[2]:
        st.markdown("<div class='admin-tab-note'>Delete requests are reviewed here.</div>", unsafe_allow_html=True)
        del_req = read_sheet("delete_requests")
        st.dataframe(del_req, use_container_width=True)

    with tabs[3]:
        st.markdown("<div class='admin-tab-note'>Google Sheet only storage. No local Excel, no sync queue, no cloud Excel tabs.</div>", unsafe_allow_html=True)
        st.success("Google Sheet only mode is active.")

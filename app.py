...
VERSION_TEXT = "Google Excel Sheet Store Only Version"
...
APP_STORE_ONLY_MODE = True
APP_EDIT_ADD_DELETE_DISABLED = True
EXISTING_EXCEL_DATA_SAFE = True
DIRECT_EXCEL_STORAGE_MODE = True
APP_DELETE_DISABLED = True
EXISTING_EXCEL_DATA_SAFE = True
...
def create_excel_if_missing():
    ...
    if EXCEL_FILE.exists():
        try:
            with pd.ExcelFile(EXCEL_FILE, engine="openpyxl") as xls:
                ...
                for sheet in xls.sheet_names:
                    existing[sheet] = pd.read_excel(xls, sheet_name=sheet)
        except Exception:
            existing = {}
        ...
        for sheet_name, cols in SHEETS.items():
            if sheet_name not in existing:
                existing[sheet_name] = pd.DataFrame(columns=cols)
                changed = True
            else:
                df = existing[sheet_name]
                for col in cols:
                    if col not in df.columns:
                        if sheet_name == "employees" and col == "User ID" and "Technician Name" in df.columns:
                            df[col] = df["Technician Name"].astype(str)
                        else:
                            df[col] = ""
                        changed = True
                # Store-only sheets keep only requested columns on next save, but do not destroy old file unless adding missing columns.
    ...
    default_employees = pd.DataFrame([
        {"User ID": "admin", "Password": "admin123", "Employee Name": "Admin", "Role": "Admin", "Status": "Active"},
        {"User ID": "manager", "Password": "manager123", "Employee Name": "Manager", "Role": "Manager", "Status": "Active"},
        {"User ID": "mohan", "Password": "1234", "Employee Name": "Mohan", "Role": "Technician", "Status": "Active"},
        {"User ID": "ajay", "Password": "1234", "Employee Name": "Ajay", "Role": "Technician", "Status": "Active"},
        {"User ID": "vengadesh", "Password": "1234", "Employee Name": "Vengadesh", "Role": "Technician", "Status": "Active"},
        {"User ID": "prathisha", "Password": "1234", "Employee Name": "Prathisha", "Role": "Prathisha / System Staff", "Status": "Active"},
    ])

    default_settings = pd.DataFrame([
        ["Version", VERSION_TEXT],
        ["Storage Type", "Google Excel Sheet Store Only"],
        ["App Edit/Add/Delete", "Disabled"],
        ["Existing Data Safety", "Code changes will not overwrite existing Excel rows"],
    ], columns=["Key", "Value"])

    with pd.ExcelWriter(EXCEL_FILE, engine="openpyxl") as writer:
        for sheet_name, cols in SHEETS.items():
            if sheet_name == "employees":
                default_employees.to_excel(writer, index=False, sheet_name=sheet_name[:31])
            elif sheet_name == "settings":
                default_settings.to_excel(writer, index=False, sheet_name=sheet_name[:31])
            else:
...
# ============================================================
# FAST EXCEL READ CACHE
# ============================================================
@st.cache_data(ttl=15, show_spinner=False)
def cached_read_excel_sheet(file_path_str, file_mtime, sheet_name):
    try:
        return pd.read_excel(file_path_str, sheet_name=sheet_name)
    except Exception:
...
def read_sheet(sheet_name):
    ...
    try:
        if "cached_read_excel_sheet" in globals() and EXCEL_FILE.exists():
            df = cached_read_excel_sheet(str(EXCEL_FILE), EXCEL_FILE.stat().st_mtime, sheet_name).copy()
        else:
            df = pd.read_excel(EXCEL_FILE, sheet_name=sheet_name)
    except Exception:
        df = pd.DataFrame(columns=SHEETS.get(sheet_name, []))

    if sheet_name == "employees" and "User ID" not in df.columns and "Technician Name" in df.columns:
        df["User ID"] = df["Technician Name"].astype(str)

    for col in SHEETS.get(sheet_name, []):
        if col not in df.columns:
            df[col] = ""
...
def write_sheet(sheet_name, df):
    """
    Robust Excel + Google Sheet store:
    - Updates local Excel selected sheet safely.
    - Keeps other sheets unchanged.
    - Saves current sheet immediately to Google Excel Sheet when secrets are configured.
    ...
    required_cols = SHEETS.get(sheet_name, list(df.columns))
    for col in required_cols:
        if col not in df.columns:
            df[col] = ""

    # Store-only requested columns for main data sheets.
    if sheet_name in ["attendance", "invoices", "manual_invoices"]:
        save_cols = [c for c in required_cols if c in df.columns]
    ...
    with pd.ExcelWriter(EXCEL_FILE, engine="openpyxl", mode="w") as writer:
        for s, data in all_sheets.items():
            if s in SHEETS and s in ["attendance", "invoices", "manual_invoices"]:
                cols = SHEETS[s]
                for col in cols:
                    if col not in data.columns:
                        data[col] = ""
                data = data[[c for c in cols if c in data.columns]]
            data.to_excel(writer, index=False, sheet_name=s[:31])

    if "clear_excel_cache" in globals():
        clear_excel_cache()

    # Important fix: Google Excel Sheet-la immediate-aa save pannum.
    if sheet_name in ["attendance", "invoices", "manual_invoices", "employees", "settings"]:
        ok, msg = sync_dataframe_to_google_sheet_immediate(sheet_name, df_to_save)
        if not ok:
            # Don't stop local save, but show visible issue.
            try:
                st.warning("Local Excel saved. Google Excel Sheet store failed: " + str(msg))
            except Exception:
                pass
...
def append_row(sheet_name, row_dict):
    """
    Store row to Excel + Google Sheet immediately.
    """
    create_excel_if_missing()
    ...
    if sheet_name in SHEETS:
        required_cols = SHEETS[sheet_name]
    else:
    ...
    if sheet_name in ["attendance", "invoices", "manual_invoices"]:
        row = {col: row_dict.get(col, "") for col in required_cols}
    ...
    try:
        st.session_state[f"last_saved_{sheet_name}_rows"] = len(df)
    except Exception:
        pass
...
# ============================================================
# IMMEDIATE GOOGLE EXCEL SHEET STORE
# ============================================================
def sync_dataframe_to_google_sheet_immediate(sheet_name, df):
    """
    Save the given dataframe immediately to Google Excel Sheet.
    This avoids app-la row show aagitu Google Sheet-la missing issue.
    """
    try:
        # Google secrets not configured na local Excel mattum save aagum.
        if "is_google_sync_configured" in globals():
            if not is_google_sync_configured():
                return False, "Google Sheet secrets not configured."
        elif "is_google_auto_sync_enabled" in globals():
            if not is_google_auto_sync_enabled():
                return False, "Google Sheet secrets not configured."
        else:
            return False, "Google sync function not found."

        if "google_sheet_client_for_sync" in globals():
            client, err = google_sheet_client_for_sync()
        elif "google_sheet_client" in globals():
            client, err = google_sheet_client()
        else:
            return False, "Google client function not found."
        ...
        sheet_id = st.secrets.get("SHEET_ID", "")
        if not sheet_id:
            return False, "SHEET_ID missing in Streamlit Secrets."

        spreadsheet = client.open_by_key(sheet_id)

        if "get_or_create_google_worksheet" in globals():
        ...
        elif "get_or_create_worksheet" in globals():
        ...
        clean_df = df.copy().fillna("").astype(str)
        ...
        try:
            st.session_state["last_google_store_status"] = f"{sheet_name} saved to Google Excel Sheet"
            st.session_state["last_google_store_time"] = app_now().strftime("%d-%m-%Y %I:%M:%S %p")
        except Exception:
            pass

        return True, f"{sheet_name} saved to Google Excel Sheet"
    except Exception as e:
        try:
            st.session_state["last_google_store_status"] = "Google Excel store failed: " + str(e)
        except Exception:
            pass
        return False, str(e)
...
def login_user(user_id, password):
    ...
    users["Technician Name"] = users["Technician Name"].astype(str)
    users["Password"] = users["Password"].astype(str)
    users["Status"] = users["Status"].astype(str)

    match = users[
        (users["Technician Name"].str.strip() == str(user_id).strip()) &
...
def create_pending_invoice_request(data):
    ...
    append_row("pending_invoice_requests", {
        ...
        "Time": time_str(),
        "Technician Name": st.session_state.get("employee_name", ""),
        "Technician Name": st.session_state.get("user_id", ""),
...
def save_invoice_entry_from_data(data, entry_type="OCR"):
    ...
    job_card = str(data.get("Job Card Number", "")).strip()
    reg_no = reg_no_or_fr(data.get("Registration Number", ""))
    bike_model = str(data.get("Bike Model", "")).strip()
    ...
    st.success("Invoice entry stored in Excel sheet.")
...
def generate_attendance_report_pdf(df, title, file_name):
    ...
    headers = ["S.No", "Date", "Time", "Technician Name", "Name", "Role", "Status", "Distance"]
    ...
    if df.empty:
        ...
        for idx, row in show_df.iterrows():
            ...
            vals = [
                ...
                str(row.get("Date", ""))[:10],
                str(row.get("Time", ""))[:12],
                str(row.get("Technician Name", ""))[:10],
...
def generate_manual_bill_pdf(customer_name, reg_no, bike_model, spare_rows, labour_amount, service_type):
    ...
    hero_waiting_loader("Saving manual bill to Excel sheet", 3)
...
def hero_waiting_loader(message="Processing", seconds=0):
    """
    Full-screen HERO loading overlay.
    Fast mode: no forced waiting/sleep. It only shows a visible loading screen.
    """
    st.markdown(f"""
    <div class="hero-fullscreen-loader">
        <div class="hero-loader-card">
            <div class="hero-loader-logo">HERO</div>
            <div class="hero-loader-spinner"></div>
            <div class="hero-loader-title">{message}</div>
            <div class="hero-loader-sub">Please wait... app is processing your request.</div>
            <div class="hero-loader-time">Expected waiting time: 2 - 10 seconds</div>
        </div>
    </div>
    <style>
    .hero-fullscreen-loader {{
        position: fixed;
        inset: 0;
        z-index: 999999;
    ...
    .hero-loader-logo {{
        ...
        letter-spacing: 2px;
        margin-bottom: 14px;
        animation: heroPulse 1.2s infinite ease-in-out;
    }}
    .hero-loader-spinner {{
        width: 54px;
        height: 54px;
...
# ============================================================
# SAFE TECHNICIAN FILTER HELPERS
# ============================================================
...
def filter_entries_for_current_user(df):
    ...
    user_id = str(st.session_state.get("user_id", "")).strip().lower()
    employee_name = str(st.session_state.get("employee_name", "")).strip().lower()

    if "User ID" in df.columns and user_id:
        return df[df["User ID"].astype(str).str.strip().str.lower() == user_id]

    if "Technician Name" in df.columns and employee_name:
        return df[df["Technician Name"].astype(str).str.strip().str.lower() == employee_name]
...
def page_login():
    ...
    with c2:
        ...
        remember = st.checkbox("Remember my Technician Name", value=False)
        ...
        if st.button("Login", use_container_width=True):
            ...
            if user:
                ...
                st.session_state["user_id"] = user["Technician Name"]
...
def page_dashboard():
    ...
    page_hero(
        ...
        "Store-only dashboard. Data is filtered safely without delete request logic.",
        employee_name
    )
    ...
    # Technician sees only own User ID entries; Admin/Manager sees all
    user_invoices = filter_entries_for_current_user(invoices)
    today_df = today_entries_for_current_user(invoices)

    if is_admin() or is_manager():
        title_caption = "All entries"
    else:
        title_caption = "Your entries only"

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        metric_card("Today Revenue", f"₹{today_df['Total Amount'].sum():,.0f}" if not today_df.empty else "₹0", title_caption)
    with c2:
        metric_card("Today Vehicle", len(today_df), "Today completed")
    with c3:
        metric_card("Labour Amount", f"₹{today_df['Labour Amount'].sum():,.0f}" if not today_df.empty else "₹0", "Today labour")
    with c4:
        metric_card("Store Mode", "Active", "Google Excel Sheet")
    ...
    existing_cols = [c for c in show_cols if c in today_df.columns]
    if today_df.empty:
        st.info("Today vehicle entries illa.")
    else:
        st.dataframe(today_df[existing_cols] if existing_cols else today_df, use_container_width=True)
    ...
    existing_cols = [c for c in show_cols if c in recent.columns]
    if recent.empty:
        st.info("Service entries illa.")
    else:
        st.dataframe(recent[existing_cols] if existing_cols else recent, use_container_width=True)

    st.markdown("<div class='section-title'>Attendance Status</div>", unsafe_allow_html=True)
    if not attendance.empty:
        att_df = attendance.copy()
        if not (is_admin() or is_manager()):
            uid = str(st.session_state.get("user_id", "")).strip().lower()
            emp = str(st.session_state.get("employee_name", "")).strip().lower()
            if "User ID" in att_df.columns:
                att_df = att_df[att_df["User ID"].astype(str).str.strip().str.lower() == uid]
            elif "Technician Name" in att_df.columns:
                att_df = att_df[att_df["Technician Name"].astype(str).str.strip().str.lower() == emp]
        att_cols = [c for c in ["Date", "Time", "User ID", "Technician Name", "Attendance Status"] if c in att_df.columns]
        st.dataframe(att_df.tail(10)[att_cols] if att_cols else att_df.tail(10), use_container_width=True)
    else:
        st.info("Attendance entries illa.")
...
def page_attendance():
    ...
    exists = att[
        ...
        (att["Technician Name"].astype(str) == user_id)
    ]
...
def page_reports():
    ...
    invoices = read_sheet("invoices")
    invoices = filter_entries_for_current_user(invoices)
        ...
        if invoices.empty:
            ...
            if is_technician():
                service_df = service_df[
                    (service_df["Technician Name"].astype(str) == st.session_state.get("user_id", "")) &
    ...
    with tabs[1]:
        ...
        if is_technician():
            ...
            daily_df = daily_df_base[
                ...
                (daily_df_base["Technician Name"].astype(str) == st.session_state.get("user_id", ""))
            ]
    ...
    with tabs[2]:
        ...
        if attendance_df.empty:
            ...
            if is_technician() or is_prathisha():
                ...
                att_df = att_df[att_df["Technician Name"].astype(str) == st.session_state.get("user_id", "")]
            ...
            att_cols = ["Date", "Time", "Technician Name", "Technician Name", "Role", "Attendance Status", "Distance Meter", "Selfie Saved"]
...
def page_search():
    ...
    safe_cols = [
        "Entry ID", "Date", "Technician Name", "Technician Name", "Invoice Number",
...
def page_customer_service_history():
    page_hero(
        "Customer Service History",
        "Search customer, vehicle and service history from stored invoice entries.",
    ...
    # New store-only invoice sheet does not keep Technician Name.
    # Technician users will see their own technician-name entries.
    if is_technician():
        tech_name = str(st.session_state.get("employee_name", "")).strip().lower()
        if "Technician Name" in invoices.columns:
            invoices = invoices[invoices["Technician Name"].astype(str).str.strip().str.lower() == tech_name]
        else:
            invoices = invoices.iloc[0:0]

    st.markdown("<div class='section-title'>Search Service History</div>", unsafe_allow_html=True)

    search_text = st.text_input(
        "Search by Registration Number / Job Card Number / Invoice Number / Bike Model",
        key="customer_history_search"
    )
    ...
    if search_text:
        ...
        search_cols = [
            "Registration Number", "Job Card Number", "Invoice Number",
            "Bike Model", "Technician Name", "Service Type"
        ]
        ...
        if existing_cols:
            ...
            for col in existing_cols:
                col_mask = filtered[col].astype(str).str.lower().str.contains(q, na=False)
                mask = col_mask if isinstance(mask, bool) else (mask | col_mask)
            filtered = filtered[mask]

    show_cols = [
        "Date", "Technician Name", "Invoice Number", "Job Card Number",
        "Registration Number", "Bike Model", "Service Type",
        "Labour Amount", "Spare Amount", "Oil Change Status", "Total Amount"
    ]
    existing_show_cols = [c for c in show_cols if c in filtered.columns]

    if filtered.empty:
        st.info("No service history found.")
    else:
        st.dataframe(filtered[existing_show_cols] if existing_show_cols else filtered, use_container_width=True)
...
def page_delete_invoice_request():
    st.error("Delete request removed. App-la delete panna mudiyathu.")
    return
...
# ============================================================
# DIRECT EXCEL SHEET DATA MANAGER
# ============================================================
def admin_excel_data_manager():
    st.error("Excel data edit/add/delete disabled in app. Direct Excel sheet-la mattum changes pannunga.")
    return
...
def page_admin_panel():
    page_hero(
        "Admin Panel",
        "Google Excel Sheet store-only control center. App-la edit/add/delete/download options disabled.",
        "Admin"
    )

    premium_panel(
        "Store Only Mode",
        "App only stores attendance, invoice and manual bill entries. Excel data edit/add/delete must be done outside app directly in the sheet."
    )
    ...
    with tabs[0]:
        st.markdown(
            "<div class='admin-tab-note'>Revenue overview from stored Excel sheet data. No app edit/delete options.</div>",
            unsafe_allow_html=True
        )

        c1, c2, c3 = st.columns(3)
        with c1:
            ultra_card("Total Revenue", f"₹{invoices['Total Amount'].sum():,.0f}", "Stored entries", "💰")
        with c2:
            ultra_card("Invoice Entries", len(invoices), "Store only", "🧾")
        with c3:
            ultra_card("Mode", "Store Only", "No app edit/delete", "🔒")
    ...
    with tabs[1]:
        st.markdown(
            "<div class='admin-tab-note'>Employee Add/Edit removed from app. Employee changes direct-aa Excel sheet-la mattum pannunga.</div>",
            unsafe_allow_html=True
        )
        employees = read_sheet("employees")
        st.dataframe(employees, use_container_width=True)
        st.info("App-la Employee Add/Edit disabled. Direct Excel sheet-la mattum employee edit pannunga.")

    with tabs[2]:
        st.markdown(
            "<div class='admin-tab-note'>Settings download/storage controls removed. App is store-only.</div>",
            unsafe_allow_html=True
        )
        st.success("Google Excel Sheet Store Only mode active.")
        st.info("Code change pannalum existing Excel sheet rows affect aagathu. Existing workbook irundha overwrite panna maatom.")

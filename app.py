...
def page_delete_invoice_request():
    st.error("Delete Invoice Request feature removed. App moolama delete panna mudiyathu.")
    return
...
def admin_excel_data_manager():
    st.markdown(
        "<div class='premium-title-card'><h2>Direct Excel Sheet Data</h2><p>View, edit and add rows directly to Excel file. Delete disabled in app.</p></div>",
        unsafe_allow_html=True
    )
    st.caption("Data direct-aa data/selva_motors_excel_storage.xlsx file-la store aagum. App moolama delete panna mudiyathu. Code change pannalum existing Excel data overwrite aagathu.")

    sheet_name = st.selectbox(
        "Select Excel Sheet",
        list(SHEETS.keys()),
        key="direct_excel_sheet_select"
    )

    df = read_sheet(sheet_name).reset_index(drop=True)

    c1, c2, c3 = st.columns(3)
    c1.metric("Sheet", sheet_name)
    c2.metric("Rows", len(df))
    c3.metric("Columns", len(df.columns))
    ...
    pwd = st.text_input(
        "Enter password to unlock edit/add",
        type="password",
        key=f"direct_excel_pwd_{sheet_name}"
    )

    if pwd != SECRET_PASSWORD:
        if pwd:
            st.error("Wrong password.")
        st.info("Password enter pannina direct Excel edit/add options show aagum. Delete option app-la disabled.")
        return

    st.success("Direct Excel edit/add unlocked. Delete option disabled in app.")

    action = st.radio(
        "Select Action",
        ["Edit Row", "Add Row"],
        horizontal=True,
        key=f"direct_excel_action_{sheet_name}"
    )
    ...
    if action == "Add Row":
        ...
        for col in SHEETS[sheet_name]:
            ...
            if col == "Date":
                ...
                default = "E-" + uuid.uuid4().hex[:8].upper()
            elif col == "Request ID":
                default = "R-" + uuid.uuid4().hex[:8].upper()

            new_row[col] = st.text_input(
                col,
                value=default,
                key=f"direct_add_{sheet_name}_{col}"
            )

        if st.button("Add Row Directly to Excel", use_container_width=True, key=f"direct_add_btn_{sheet_name}"):
            append_row(sheet_name, new_row)
            st.success("New row added directly to Excel sheet.")
            st.rerun()
        ...
        idx = st.selectbox(
            ...
            key=f"direct_edit_idx_{sheet_name}"
        )

        edited = {}
        for col in SHEETS[sheet_name]:
            edited[col] = st.text_input(
                col,
                value=str(df.iloc[idx].get(col, "")),
                key=f"direct_edit_{sheet_name}_{idx}_{col}"
            )

        if st.button("Save Edited Row Directly to Excel", use_container_width=True, key=f"direct_save_edit_{sheet_name}"):
            fresh = read_sheet(sheet_name).reset_index(drop=True)
            if idx >= len(fresh):
                st.error("Row not found after refresh.")
                return

            for col in SHEETS[sheet_name]:
                fresh.loc[idx, col] = edited[col]

            write_sheet(sheet_name, fresh)
            st.success("Row updated directly in Excel sheet.")
            st.rerun()

    st.divider()
    st.info("Delete option disabled in app. Row delete panna vendumna Excel file-a outside app manually edit pannunga.")

    if EXCEL_FILE.exists():
        with open(EXCEL_FILE, "rb") as f:
            st.download_button(
                "Download Direct Excel Sheet",
                f,
                file_name="selva_motors_excel_storage.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
                key=f"direct_excel_download_{sheet_name}"
            )
...
def page_admin_panel():
    page_hero(
        "Admin Panel",
        "Direct Excel Sheet control center: revenue, employees, direct Excel data manager and settings.",
        "Admin"
    )

    premium_panel(
        "Admin Control Center",
        "Direct Excel storage, reports, customer history, employee control and Excel edit/delete."
    )
    ...
    with tabs[0]:
        st.markdown(
            "<div class='admin-tab-note'>Revenue overview from Direct Excel Sheet data.</div>",
            unsafe_allow_html=True
        )
    ...
    with tabs[1]:
        st.markdown(
            "<div class='admin-tab-note'>Add or update employee login details and roles.</div>",
            unsafe_allow_html=True
        )
    ...
    with tabs[2]:
        st.markdown(
            "<div class='admin-tab-note'>Direct Excel Sheet Data: Edit/Add only here. Delete disabled in app. Existing Excel data will not be overwritten by code changes.</div>",
            unsafe_allow_html=True
        )
    ...
    with tabs[3]:
        st.markdown(
            "<div class='admin-tab-note'>Direct Excel settings and download.</div>",
            unsafe_allow_html=True
        )
        ...
        st.subheader("Direct Excel Sheet File")

...
def get_hero_logo_base64():
    try:
        if HERO_LOGO_PATH.exists():
            return base64.b64encode(HERO_LOGO_PATH.read_bytes()).decode("utf-8")
    except Exception:
        pass
    return ""
...
def page_login():
    ...
    logo_html = f'<img class="hero-login-logo-img" src="data:image/jpeg;base64,{logo_b64}" />' if logo_b64 else '<div style="font-size:46px;text-align:center;">🏍️</div>'
...
def menu_page():
    ...
    sidebar_logo_html = f'<img src="data:image/jpeg;base64,{sidebar_logo_b64}" style="width:150px;max-width:100%;background:#fff;border-radius:18px;padding:8px;box-shadow:0 16px 32px rgba(227,24,55,.22);" />' if sidebar_logo_b64 else '<div style="font-size:28px;">🏍️</div>'

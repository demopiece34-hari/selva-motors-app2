...
# ============================================================
# SELVA MOTORS DIRECT EXCEL SHEET APP
# Excel only. No MySQL. No SQLAlchemy. No SQL database.
# ============================================================

st.set_page_config(
    page_title="Selva Motors Direct Excel Sheet App",
    page_icon="🏍️",
    layout="wide"
)

APP_TITLE = "SELVA MOTORS DIRECT EXCEL SHEET APP"
VERSION_TEXT = "Direct Excel Sheet Storage Version"
...
DIRECT_EXCEL_STORAGE_MODE = True
GOOGLE_SHEET_PRIMARY_STORAGE = True
LOCAL_EXCEL_BACKUP_ONLY = True
CODE_CHANGE_SAFE_NO_DATA_DELETE = True
...
SHEETS = {
    ...
    "attendance": ["Date", "Time", "User ID", "Technician Name", "Attendance Status"],
    "invoices": [
        "Date", "User ID", "Technician Name", "Invoice Number", "Job Card Number",
        "Registration Number", "Bike Model", "Service Type",
        "Labour Amount", "Spare Amount", "Oil Change Status", "Total Amount"
    ],
    "manual_invoices": [
        "Date", "User ID", "Technician Name", "Customer Name", "Registration Number",
        "Bike Model", "Service Type", "Labour Amount", "Spare Parts Count",
        "Oil Count", "Total Amount", "Status"
    ],
    "settings": ["Key", "Value"]
}
...
# ============================================================
# GOOGLE PRIMARY STORAGE + SAFE UI HELPERS
# ============================================================
def hero_fullscreen_loader(message="Saving to Google Sheet", seconds=0):
    st.markdown(f"""
    <style>
    .hero-fullscreen-wait {{
        ...
        background: linear-gradient(135deg, rgba(255,255,255,.98), rgba(255,241,242,.98)) !important;
        backdrop-filter: blur(10px) !important;
    }}
    .hero-wait-card {{
        width:min(460px,92vw); padding:36px 30px; border-radius:34px; background:#fff;
        border:2px solid #fecaca; box-shadow:0 32px 95px rgba(127,29,29,.28);
        text-align:center; font-family:Arial,sans-serif;
    }}
    .hero-wait-logo {{ color:#e31837; font-size:56px; font-weight:1000; letter-spacing:3px; margin-bottom:18px; animation:heroPulse 1.1s infinite ease-in-out; }}
    .hero-wait-spinner {{ width:72px; height:72px; border:7px solid #fee2e2; border-top:7px solid #e31837; border-radius:999px; margin:0 auto 20px auto; animation:heroSpin .75s linear infinite; }}
    .hero-wait-title {{ font-size:20px; font-weight:900; color:#111827; }}
    .hero-wait-sub {{ color:#4b5563; font-size:14px; margin-top:10px; font-weight:650; }}
    .hero-wait-time {{ display:inline-block; margin-top:16px; padding:9px 14px; border-radius:999px; background:#fff1f2; color:#991b1b; border:1px solid #fecdd3; font-weight:900; font-size:14px; }}
    @keyframes heroSpin {{ from {{transform:rotate(0deg)}} to {{transform:rotate(360deg)}} }}
    @keyframes heroPulse {{ 0%,100% {{transform:scale(1);opacity:1}} 50% {{transform:scale(1.08);opacity:.76}} }}
    </style>
    <div class="hero-fullscreen-wait"><div class="hero-wait-card">
    <div class="hero-wait-logo">HERO</div><div class="hero-wait-spinner"></div>
    <div class="hero-wait-title">{message}</div>
    <div class="hero-wait-sub">Please wait... Google Sheet update is processing.</div>
    <div class="hero-wait-time">Expected waiting time: 2 - 10 seconds</div>
    </div></div>
    """, unsafe_allow_html=True)

def hero_waiting_loader(message="Processing", seconds=0):
    hero_fullscreen_loader(message, seconds)

def clean_duplicate_columns(df):
    if df is None: return pd.DataFrame()
    return df.copy().loc[:, ~pd.Index(df.columns).duplicated()]

def safe_existing_cols(df, cols):
...
def safe_dataframe(df, use_container_width=True):
    st.dataframe(clean_duplicate_columns(df), use_container_width=use_container_width)

@st.cache_data(ttl=10, show_spinner=False)
def cached_local_excel_read(file_path_str, file_mtime, sheet_name):
    try: return pd.read_excel(file_path_str, sheet_name=sheet_name)
    except Exception: return pd.DataFrame(columns=SHEETS.get(sheet_name, []))

def clear_storage_cache():
    try: cached_local_excel_read.clear()
    except Exception: pass

def get_google_sheet_id():
    try: return st.secrets.get("SHEET_ID","") or st.secrets.get("GOOGLE_SHEET_ID","")
    except Exception: return ""

def get_google_client_primary():
    try:
        ...
        if "gcp_service_account" in st.secrets:
            ...
            return None, "gcp_service_account secret missing"
        creds=Credentials.from_service_account_info(info, scopes=scopes)
        return gspread.authorize(creds), None
    except Exception as e:
        return None, str(e)

def get_or_create_google_worksheet_primary(spreadsheet, sheet_name, min_rows=100, min_cols=20):
    try: ws=spreadsheet.worksheet(sheet_name)
    except Exception: ws=spreadsheet.add_worksheet(title=sheet_name, rows=min_rows, cols=min_cols)
    try:
...
def google_read_sheet(sheet_name):
    client, err = get_google_client_primary()
    sid = get_google_sheet_id()
    if client is None or not sid: return None, err or "SHEET_ID missing"
    try:
        ss=client.open_by_key(sid)
        cols=SHEETS.get(sheet_name, [])
        ws=get_or_create_google_worksheet_primary(ss, sheet_name, 100, max(len(cols)+5,20))
        values=ws.get_all_values()
        if not values:
            if cols: ws.update([cols], value_input_option="USER_ENTERED")
        ...
        if missing:
            ...
            ws.clear()
            ws.update([ordered]+df.fillna("").astype(str).values.tolist(), value_input_option="USER_ENTERED")
        return df, None
    except Exception as e:
        return None, str(e)

def google_write_sheet(sheet_name, df, allow_delete_update=True):
    client, err = get_google_client_primary()
    sid=get_google_sheet_id()
    if client is None or not sid: return False, err or "SHEET_ID missing"
    try:
        df=clean_duplicate_columns(df).copy()
        cols=SHEETS.get(sheet_name, list(df.columns))
        for c in cols:
            if c not in df.columns: df[c]=""
        ordered=[c for c in cols if c in df.columns]+[c for c in df.columns if c not in cols]
        df=df[ordered]
        ss=client.open_by_key(sid)
        ws=get_or_create_google_worksheet_primary(ss, sheet_name, max(len(df)+30,100), max(len(df.columns)+5,20))
...
def google_append_row(sheet_name, row_dict):
    client, err = get_google_client_primary()
    sid=get_google_sheet_id()
    if client is None or not sid: return False, err or "SHEET_ID missing"
    try:
        cols=SHEETS.get(sheet_name, list(row_dict.keys()))
        ss=client.open_by_key(sid)
        ws=get_or_create_google_worksheet_primary(ss, sheet_name, 100, max(len(cols)+5,20))
        values=ws.get_all_values()
        if not values:
            ...
            if missing:
                old_rows=values[1:]
                old=pd.DataFrame(old_rows, columns=headers) if old_rows else pd.DataFrame(columns=headers)
                for c in missing: old[c]=""
                headers=headers+missing
                ws.clear()
                ws.update([headers]+old.fillna("").astype(str).values.tolist(), value_input_option="USER_ENTERED")
        ws.append_row([str(row_dict.get(c,"")) for c in headers], value_input_option="USER_ENTERED")
        clear_storage_cache()
        return True, "Saved directly to Google Sheet"
    except Exception as e:
        return False, str(e)

def local_backup_write(sheet_name, df):
    try:
        DATA_DIR.mkdir(exist_ok=True)
        existing={}
            ...
            try:
                with pd.ExcelFile(EXCEL_FILE, engine="openpyxl") as xls:
                    for s in xls.sheet_names:
                        existing[s]=pd.read_excel(xls, sheet_name=s)
            except Exception: existing={}
        for s, cols in SHEETS.items():
            if s not in existing: existing[s]=pd.DataFrame(columns=cols)
        existing[sheet_name]=clean_duplicate_columns(df)
        with pd.ExcelWriter(EXCEL_FILE, engine="openpyxl", mode="w") as writer:
            for s,d in existing.items(): d.to_excel(writer, index=False, sheet_name=s[:31])
    except Exception: pass

def filter_entries_for_current_user(df):
    if df is None or df.empty: return df
    if is_admin() or is_manager(): return df
    uid=str(st.session_state.get("user_id","")).strip().lower()
    emp=str(st.session_state.get("employee_name","")).strip().lower()
    if "User ID" in df.columns and uid:
        return df[df["User ID"].astype(str).str.strip().str.lower()==uid]
    if "Technician Name" in df.columns and emp:
        return df[df["Technician Name"].astype(str).str.strip().str.lower()==emp]
    return df.iloc[0:0]

def today_entries_for_current_user(df):
    df=filter_entries_for_current_user(df)
    if df is None or df.empty: return df
    if "Date" in df.columns:
        return df[df["Date"].astype(str)==today_str()]
    return df.iloc[0:0]

def create_excel_if_missing():
    """
    Google Sheet only mode:
    - Only create folders needed by the app.
    - Do not create local Excel.
    - Do not scan/read every Google sheet on startup.
    """
    DATA_DIR.mkdir(exist_ok=True)
    UPLOAD_DIR.mkdir(exist_ok=True)
    PDF_DIR.mkdir(exist_ok=True)
...
def read_sheet(sheet_name):
    """
    Google Sheet only read. No local Excel fallback.
    This function should be called only after user clicks SHOW on a page.
    """
    df, err = google_read_sheet(sheet_name)
    if df is not None:
        return clean_duplicate_columns(df)

    # Return an empty dataframe with expected columns if Google read fails.
    return pd.DataFrame(columns=SHEETS.get(sheet_name, []))



def write_sheet(sheet_name, df):
    df = clean_duplicate_columns(df)
    for c in SHEETS.get(sheet_name, []):
        if c not in df.columns:
            df[c] = ""
    hero_fullscreen_loader("Updating Google Sheet", 0)
    ok, msg = google_write_sheet(sheet_name, df, allow_delete_update=True)
    if ok:
        try:
        ...
        try:
            st.error("Google Sheet update failed: " + str(msg))
        except Exception:
            pass
    clear_storage_cache()
...
def append_row(sheet_name, row_dict):
    """
    Direct Google Sheet append only.
    Nothing is written to local Excel.
    """
    create_excel_if_missing()
    cols = SHEETS.get(sheet_name, list(row_dict.keys()))
    clean_row = {c: row_dict.get(c, "") for c in cols}
    hero_fullscreen_loader("Saving to Google Sheet", 0)
    ok, msg = google_append_row(sheet_name, clean_row)
    if ok:
        try:
            st.success("Saved directly to Google Sheet.")
        except Exception:
            pass
    else:
        try:
            st.error("Google Sheet save failed: " + str(msg))
        except Exception:
            pass
...
def pull_single_google_sheet_to_excel(sheet_name):
    """
    Pull one Google worksheet into local Excel Sheet sheet.
    Useful when user edits/deletes rows directly in Excel Sheet and wants app Excel to match.
    """
    if not is_google_auto_sync_enabled():
        return False, "Direct Excel sheet storage active. Cloud sync disabled."
    ...
    try:
        ...
        try:
            ...
            return False, f"Excel Sheet worksheet not found: {sheet_name}"
        ...
        try:
            ...
            state["last_sync_status"] = "Pulled from Excel Sheet"
            state["last_sync_time"] = app_now().strftime("%d-%m-%Y %I:%M:%S %p")
            state["last_sync_message"] = f"{sheet_name} pulled from Excel Sheet to Excel Sheet"
            save_sync_state(state)
        except Exception:
            pass

        return True, f"{sheet_name} pulled from Excel Sheet to Excel Sheet. Rows: {len(new_df)}"
    except Exception as e:
        return False, str(e)
...
def pull_all_google_sheets_to_excel():
    """
    Pull all worksheets from Excel Sheet into Excel Sheet.
    This overwrites matching Excel sheets with Excel Sheet data.
    """
    if not is_google_auto_sync_enabled():
        return False, "Direct Excel sheet storage active. Cloud sync disabled."
    ...
    return True, "Pulled all Excel Sheet data to Excel Sheet: " + ", ".join(pulled)



# ============================================================
# GOOGLE SHEET BACKUP SYNC
# Primary storage is still Excel. Excel Sheet is only backup/sync.
# ============================================================
...
def is_google_auto_sync_enabled():
    """
    Excel Sheet auto store will work only when Streamlit secrets are configured.
    Excel remains primary storage, Excel Sheet is automatic cloud copy.
    """
...
def sync_single_excel_sheet_to_google_sheet(sheet_name):
    """
    Sync only the changed Excel sheet to Excel Sheet immediately after saving.
    This is faster than syncing all sheets after every entry.
    """
    if not is_google_auto_sync_enabled():
        return False, "Excel Sheet secrets not configured"
    ...
    try:
        ...
        return True, f"{sheet_name} synced to Excel Sheet"
    except Exception as e:
        return False, str(e)
...
def get_google_next_sync_wait_text():
    """
    Returns readable waiting time for next 3-minute Excel Sheet sync.
    """
    try:
        state = load_google_sync_state()
        dirty_sheets = state.get("dirty_sheets", [])
...
def get_sync_status_badge_text():
    ...
    if dirty_sheets:
        return "Waiting for Excel Sheet update"

    if status == "Success":
        return "Updated to Excel Sheet"
...
def auto_sync_google_sheet_3min():
    ...
    Excel Sheet sync runs only once every 3 minutes when app opens/reruns.
    """
    try:
        state = load_google_sync_state()
        dirty_sheets = state.get("dirty_sheets", [])
...
def sync_all_excel_to_google():
    """
    Copies all local Excel sheets to Excel Sheet.
    ...
    try:
        ...
        return True, "Synced to Excel Sheet: " + ", ".join(synced)
...
def save_invoice_entry_from_data(data, entry_type="OCR"):
    append_row("invoices", {
        "Date": today_str(),
        "User ID": st.session_state.get("user_id", ""),
        "Technician Name": st.session_state.get("employee_name", ""),
        "Invoice Number": str(data.get("Invoice Number", "")).strip(),
        "Job Card Number": str(data.get("Job Card Number", "")).strip(),
        "Registration Number": reg_no_or_fr(data.get("Registration Number", "")),
        "Bike Model": str(data.get("Bike Model", "")).strip(),
...
def generate_manual_bill_pdf(customer_name, reg_no, bike_model, spare_rows, labour_amount, service_type):
    ...
    append_row("manual_invoices", {
        ...
        "Oil Count": oil_count if "oil_count" in locals() else 0,
        ...
        <div class="login-hero">
            <div class="hero-login-panel">
                ...
                <div class="feature-strip" style="justify-content:center;margin-bottom:18px;">
                    <div class="feature-pill">Direct Excel Sheet</div>
...
def page_dashboard():
    ...
    page_hero("Service Control Dashboard", "Google Sheet is read only after you click HERO SHOW.", employee_name)

    if not require_hero_show(
        "dashboard",
        "🚀 HERO SHOW DASHBOARD",
        "Nothing is read yet. Click HERO SHOW DASHBOARD to load the required Google Sheet."
    ):
        return

    hero_fullscreen_loader("Reading Dashboard Google Sheet", 0)
    invoices = clean_duplicate_columns(read_sheet("invoices"))

    for c in ["Total Amount", "Labour Amount", "Spare Amount"]:
        if c not in invoices.columns:
            invoices[c] = 0
        invoices[c] = pd.to_numeric(invoices[c], errors="coerce").fillna(0)

    user_invoices = filter_entries_for_current_user(invoices)
    today_df = today_entries_for_current_user(invoices)

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        metric_card("Today Revenue", f"₹{today_df['Total Amount'].sum():,.0f}" if not today_df.empty else "₹0", "Today")
    with c2:
        metric_card("Today Vehicle", len(today_df), "Your ID only" if not (is_admin() or is_manager()) else "All")
    with c3:
        metric_card("Labour Amount", f"₹{today_df['Labour Amount'].sum():,.0f}" if not today_df.empty else "₹0", "Today labour")
    with c4:
        metric_card("Google Sheet", "Direct", "Primary storage")

    cols = ["Date", "User ID", "Technician Name", "Invoice Number", "Job Card Number", "Registration Number", "Bike Model", "Service Type", "Labour Amount", "Spare Amount", "Oil Change Status", "Total Amount"]
    st.markdown("<div class='section-title'>Today Vehicle Entries</div>", unsafe_allow_html=True)
    ex = safe_existing_cols(today_df, cols)
    safe_dataframe(today_df[ex] if ex else today_df, use_container_width=True) if not today_df.empty else st.info("Today vehicle entries illa.")

    st.markdown("<div class='section-title'>Recent Service Entries</div>", unsafe_allow_html=True)
    recent = user_invoices.tail(10) if not user_invoices.empty else user_invoices
    ex = safe_existing_cols(recent, cols)
    safe_dataframe(recent[ex] if ex else recent, use_container_width=True) if not recent.empty else st.info("Service entries illa.")
...
# ============================================================
def page_reports():
    page_hero("Reports", "Report data loads only after HERO SHOW.", "Report Center")
    report_center_header()

    if not require_hero_show(
        "reports",
        "📑 HERO SHOW REPORTS",
        "Nothing is read yet. Click HERO SHOW REPORTS to load the required Google Sheets."
    ):
        return

    hero_fullscreen_loader("Reading Reports Google Sheets", 0)
    ...
    with tabs[0]:
        ...
        if invoices.empty:
            ...
            if is_technician():
                ...
                with c3:
                    service_type_filter = st.selectbox("Service Type Filter", ["All", "FSC", "Paid Service", "General", "Joyride"], key="service_type_filter")
                if date_filter:
                    service_df = service_df[service_df["Date"].astype(str).str.contains(date_filter)]
                if reg_filter:
                    service_df = service_df[service_df["Registration Number"].astype(str).str.contains(reg_filter, case=False, na=False)]
                if service_type_filter != "All":
                    service_df = service_df[service_df["Service Type"].astype(str).str.contains(service_type_filter, case=False, na=False)]

                st.markdown(f"<div class='optimal-v4-note'><b>View:</b> {selected_view}</div>", unsafe_allow_html=True)

            if service_df.empty:
                st.warning("No service entries match your filter.")
            else:
                simple_df = service_df[[
                    c for c in ["Date", "Technician Name", "Invoice Number", "Job Card Number", "Registration Number", "Bike Model", "Service Type", "Labour Amount", "Spare Amount", "Oil Change Status", "Total Amount"]
                    if c in service_df.columns
                ]].copy()
                safe_dataframe(simple_df, use_container_width=True)

    with tabs[1]:
        report_control_panel("Daily Technician Report", "Simple Selva Motors style report after HERO SHOW.")
        if invoices.empty:
            st.info("No invoice entries found.")
        else:
            tech_names = sorted([x for x in invoices["Technician Name"].astype(str).unique().tolist() if x.strip()]) if "Technician Name" in invoices.columns else []
            selected_tech = st.selectbox("Technician", ["All"] + tech_names, key="daily_report_tech_select")
            if selected_tech == "All":
                report_df = invoices.copy()
            else:
                report_df = invoices[invoices["Technician Name"].astype(str) == selected_tech].copy()
            report_date = st.date_input("Report Date", value=app_now().date(), key="daily_report_date")
            report_date_str = report_date.strftime("%d-%m-%Y")
            st.dataframe(report_df.head(50), use_container_width=True)

            if st.button("Generate Daily Technician PDF", key="gen_daily_pdf_btn"):
                hero_fullscreen_loader("Generating Daily Technician PDF", 0)
                pdf = generate_daily_technician_report_pdf(report_df, report_date_str, selected_tech)
                with open(pdf, "rb") as f:
                    st.download_button("Download Technician PDF", f, file_name=Path(pdf).name, mime="application/pdf")

    with tabs[2]:
        report_control_panel("Monthly Attendance Report", "Simple Selva Motors attendance report after HERO SHOW.")
        if attendance_df.empty:
            st.info("No attendance entries found.")
        else:
            months = sorted(pd.to_datetime(attendance_df["Date"], format="%d-%m-%Y", errors="coerce").dt.strftime("%m-%Y").dropna().unique().tolist()) if "Date" in attendance_df.columns else []
            selected_month = st.selectbox("Select Month", months if months else ["No Data"], key="att_month")
            selected_tech = st.selectbox("Technician", ["All"] + sorted([x for x in attendance_df.get("Technician Name", pd.Series([], dtype=str)).astype(str).unique().tolist() if x.strip()]), key="att_tech")
            df = attendance_df.copy()
            if selected_month != "No Data" and "Date" in df.columns:
                df["Month"] = pd.to_datetime(df["Date"], format="%d-%m-%Y", errors="coerce").dt.strftime("%m-%Y")
                df = df[df["Month"] == selected_month]
            if selected_tech != "All" and "Technician Name" in df.columns:
                df = df[df["Technician Name"].astype(str) == selected_tech]
...
# ============================================================
def page_search():
    page_hero("Search Service Entries", "Search invoice, vehicle and job card entries only after HERO SHOW.", st.session_state.get("employee_name", ""))

    if not require_hero_show(
        "search",
        "🔍 HERO SHOW SEARCH",
        "Nothing is read yet. Click HERO SHOW SEARCH to load the required Google Sheet."
    ):
        return

    hero_fullscreen_loader("Reading Search Google Sheet", 0)
    invoices = filter_entries_for_current_user(clean_duplicate_columns(read_sheet("invoices")))

    q = st.text_input("Search by Invoice Number / Job Card Number / Registration Number / Bike Model / Service Type", key="safe_search_text")
    result = invoices.copy()

    if q:
        q = q.strip().lower()
        search_cols = safe_existing_cols(result, ["Date", "User ID", "Technician Name", "Invoice Number", "Job Card Number", "Registration Number", "Bike Model", "Service Type", "Oil Change Status", "Total Amount"])
        if search_cols:
    ...
    cols = ["Date", "User ID", "Technician Name", "Invoice Number", "Job Card Number", "Registration Number", "Bike Model", "Service Type", "Labour Amount", "Spare Amount", "Oil Change Status", "Total Amount"]
    ex = safe_existing_cols(result, cols)
    safe_dataframe(result[ex] if ex else result, use_container_width=True) if not result.empty else st.info("Search result illa.")
...
def page_customer_service_history():
    page_hero("Customer Service History", "History loads only after HERO SHOW.", "History")

    if not require_hero_show(
        "history",
        "🏍 HERO SHOW HISTORY",
        "Nothing is read yet. Click HERO SHOW HISTORY to load the required Google Sheet."
    ):
        return

    hero_fullscreen_loader("Reading Customer History Google Sheet", 0)
    invoices = read_sheet("invoices")
    today = today_str()
    ...
    safe_dataframe(today_entries, use_container_width=True)
    ...
    if reg:
        ...
        safe_dataframe(result, use_container_width=True)
...
def page_delete_invoice_request():
    st.error("Delete request removed. Edit/Delete/Update direct-aa Google Sheet-la pannunga.")
    return
...
def render_cloud_excel_view_only():
    ...
    <div class="cloud-view-panel">
        <h3>☁️ Excel Sheet View Only</h3>
        <p>Admin Panel-kulla view only. Edit/Delete options illa. Excel Sheet edit/delete panna Pull Excel Sheet → Excel Sheet use pannunga.</p>
    </div>
    """, unsafe_allow_html=True)

    if st.button("Refresh Excel Sheet View", use_container_width=True, key="cloud_view_refresh_admin_tab"):
        st.rerun()
    ...
    st.caption("Direct Excel Sheet view-only mode. No edit/delete buttons.")
...
def page_admin_panel():
    page_hero("Admin Panel", "Admin panel data loads only after HERO SHOW.", "Admin")

    if not require_hero_show(
        "admin",
        "⚙️ HERO SHOW ADMIN PANEL",
        "Nothing is read yet. Click HERO SHOW ADMIN PANEL to load the required Google Sheets."
    ):
        return

    hero_fullscreen_loader("Reading Admin Google Sheets", 0)
    ...
    with tabs[0]:
        ...
        if not invoices.empty and "Technician Name" in invoices.columns:
            ...
            safe_dataframe(tech, use_container_width=True)
        else:
    ...
    with tabs[1]:
        ...
        safe_dataframe(employees, use_container_width=True)

        st.markdown("""
        <div class="password-reset-panel">
            <h3>🔐 Admin Password Reset / Update</h3>
            <p>Employee password reset panna current sheet row update aagum.</p>
        </div>
        """, unsafe_allow_html=True)

        if not employees.empty:
            emp_user = st.selectbox("Employee User ID", employees["User ID"].astype(str).tolist(), key="admin_emp_select")
            new_pwd = st.text_input("New Password", type="password", key="admin_new_pwd")
            if st.button("Update Password", key="admin_pwd_btn"):
                if new_pwd.strip():
                    employees.loc[employees["User ID"].astype(str) == emp_user, "Password"] = new_pwd.strip()
                    write_sheet("employees", employees)
                    st.success("Password updated.")
                else:
                    st.warning("Enter password.")

    with tabs[2]:
        st.markdown("<div class='admin-tab-note'>Delete Requests list. Nothing is auto deleted. Admin can approve/reject requests only.</div>", unsafe_allow_html=True)
        delete_req = read_sheet("delete_requests")
        safe_dataframe(delete_req, use_container_width=True) if not delete_req.empty else st.info("No delete requests.")

    with tabs[3]:
        st.markdown("<div class='admin-tab-note'>Cloud Excel / sync status tab.</div>", unsafe_allow_html=True)
        st.caption("Google Sheet primary storage active. No local Excel view or auto cloud pull on open.")
        st.info("This section is view-only for current build.")
    ...
    with tabs[5]:
        ...
        safe_dataframe(settings, use_container_width=True)
...
def make_backup_zip():
    ...
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as z:
        # Backup app files and generated PDFs only.
        app_file = Path(__file__)
        if app_file.exists():
            z.write(app_file, app_file.name)

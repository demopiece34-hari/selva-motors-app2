...
def create_excel_if_missing():
    """
    Existing Excel data safety:
    - Existing workbook irundha overwrite panna koodathu.
    - Existing rows delete/change panna koodathu.
    - Extra old columns remove panna koodathu.
    - Missing sheets / missing required columns mattum add aagum.
    """
    DATA_DIR.mkdir(exist_ok=True)
    UPLOAD_DIR.mkdir(exist_ok=True)
    PDF_DIR.mkdir(exist_ok=True)
...
def read_sheet(sheet_name):
    ...
    if EXCEL_FILE.exists():
        ...
        df = cached_read_excel_sheet(str(EXCEL_FILE), mtime, sheet_name).copy()
    else:
        df = pd.DataFrame(columns=SHEETS.get(sheet_name, []))

    # Existing old columns are kept. Missing required columns are added.
...
def save_invoice_entry_from_data(data, entry_type="OCR"):
    ...
    hero_waiting_loader("Saving invoice entry to Excel sheet", 3)
    append_row("invoices", {
        "Date": today_str(),
        "Technician Name": st.session_state.get("employee_name", ""),

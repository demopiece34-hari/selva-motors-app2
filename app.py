...
def admin_excel_data_manager():
    ...
    <div class="bigdata-panel">
        <h3>⚡ Big Data Edit / Delete Mode</h3>
        <p>Large rows irundha first search/filter pannunga. Filtered rows-la irundhu select pannina edit/delete fast-aa easy-aa pannalam.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if action == "Add Row":
        ...
        if "display_df" in locals() and not display_df.empty:
            edit_options = display_df["Original Row Number"].astype(int).tolist()
        else:
            edit_options = list(range(1, len(df) + 1))

        selected_row_no = st.selectbox(
            "Select Filtered Row Number to Edit",
            edit_options,
            format_func=lambda i: f"Row {i}",
            key=f"edit_row_{sheet_name}"
        )
        selected_idx = int(selected_row_no) - 1
        ...
        if "display_df" in locals() and not display_df.empty:
            delete_options = display_df["Original Row Number"].astype(int).tolist()
        else:
            delete_options = list(range(1, len(df) + 1))

        selected_row_no = st.selectbox(
            "Select Filtered Row Number to Delete",
            delete_options,
            format_func=lambda i: f"Row {i}",
        ...
        st.markdown("<div class='bigdata-warning'>Filtered row select mode enabled. Preview correct-aa check pannitu DELETE type pannunga.</div>", unsafe_allow_html=True)

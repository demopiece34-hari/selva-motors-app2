...
# ============================================================
# SELVA MOTORS GOOGLE SHEET ONLY APP
# Google Sheet only. No MySQL. No SQLAlchemy. No SQL database.
# ============================================================
...
APP_TITLE = "SELVA MOTORS GOOGLE SHEET ONLY APP"
VERSION_TEXT = "Google Sheet Only Storage Version"
...
# ============================================================
# GOOGLE ONLY / ON-DEMAND LOAD HELPERS
# ============================================================
def hero_fullscreen_loader(message="Loading data..."):
    logo_b64 = get_hero_logo_base64()
    logo_html = (
        f'<img src="data:image/jpeg;base64,{logo_b64}" '
        f'style="width:170px;max-width:100%;display:block;margin:0 auto 18px auto;'
        f'border-radius:18px;box-shadow:0 18px 34px rgba(227,24,55,.18);" />'
        if logo_b64 else
        '<div style="font-size:46px;font-weight:900;color:#e31837;text-align:center;margin-bottom:18px;">HERO</div>'
    )
    st.markdown(f"""
    <style>
    .hero-loading-overlay {{
        ...
        background:
            radial-gradient(circle at 20% 0%, rgba(227,24,55,.18), transparent 28%),
            linear-gradient(135deg, rgba(255,255,255,.98), rgba(255,247,247,.98));
    ...
    .hero-loading-card {{
        ...
        box-shadow: 0 30px 80px rgba(17,24,39,.20);
        text-align: center;
    }}
    ...
    <div class="hero-loading-overlay">
        <div class="hero-loading-card">
            {logo_html}
            <div class="hero-loading-spinner"></div>
            <div class="hero-loading-title">{message}</div>
            <div class="hero-loading-sub">Please wait...</div>
        </div>
    </div>
    """, unsafe_allow_html=True)


def require_hero_show(page_key, button_text, loader_text):
    session_key = f"hero_show_{page_key}"
    if not st.session_state.get(session_key, False):
        st.markdown("""
        <div class="hero-panel">
            <div class="status-chip">HERO SHOW MODE</div>
            <h1>Nothing is read until you press the show button</h1>
            <p>Only the required Google Sheet will load after button click.</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button(button_text, use_container_width=True, key=f"{page_key}_show_btn"):
            st.session_state[session_key] = True
            st.rerun()
        return False
    hero_fullscreen_loader(loader_text)
    return True
...
def google_read_sheet(sheet_name):
    ...
    if client is None or not sheet_id:
        return pd.DataFrame(columns=SHEETS[sheet_name]), err or "Google Sheet secrets missing"
    try:
        spreadsheet = client.open_by_key(sheet_id)
        try:
            ws = spreadsheet.worksheet(sheet_name)
        except Exception:
            ws = spreadsheet.add_worksheet(title=sheet_name, rows=100, cols=max(len(SHEETS[sheet_name]) + 5, 20))
        ...
        return pd.DataFrame(columns=SHEETS[sheet_name]), str(e)


def google_write_sheet(sheet_name, df):
    sheet_id = st.secrets.get("SHEET_ID", "")
    client, err = google_sheet_client()
    if client is None or not sheet_id:
        return False, err or "Google Sheet secrets missing"
    try:
        spreadsheet = client.open_by_key(sheet_id)
        try:
            ws = spreadsheet.worksheet(sheet_name)
        except Exception:
            ws = spreadsheet.add_worksheet(title=sheet_name, rows=max(len(df) + 30, 100), cols=max(len(df.columns) + 5, 20))
        clean_df = df.copy().fillna("").astype(str)
        ws.clear()
        ws.update([clean_df.columns.tolist()] + clean_df.values.tolist(), value_input_option="USER_ENTERED")
        return True, ""
    except Exception as e:
        return False, str(e)
...
def create_excel_if_missing():
    # Local Excel removed. Keep app folders only.
    DATA_DIR.mkdir(exist_ok=True)
    UPLOAD_DIR.mkdir(exist_ok=True)
    PDF_DIR.mkdir(exist_ok=True)
    BACKUP_DIR.mkdir(exist_ok=True)

    # Ensure Google sheets exist and headers are present.
    try:
        sheet_id = st.secrets.get("SHEET_ID", "")
        client, err = google_sheet_client()
        if client is None or not sheet_id:
        ...
        for sheet_name, cols in SHEETS.items():
            try:
                ...
                ws = spreadsheet.add_worksheet(title=sheet_name, rows=100, cols=max(len(cols) + 5, 20))
...
def write_sheet(sheet_name, df):
    # Google Sheet ONLY write. No local Excel backup.
    create_excel_if_missing()
    for col in SHEETS[sheet_name]:
        if col not in df.columns:
            df[col] = ""
    df = df[SHEETS[sheet_name]].copy().fillna("")
    ok, err = google_write_sheet(sheet_name, df)
    if not ok:
        st.error(f"Google Sheet save failed: {err}")


def append_row(sheet_name, row_dict):
    # Google Sheet ONLY append.
    create_excel_if_missing()
    clean_row = {col: row_dict.get(col, "") for col in SHEETS[sheet_name]}
    ok, err = google_append_row(sheet_name, clean_row)
    if not ok:
        st.error(f"Google Sheet append failed: {err}")


def pull_single_google_sheet_to_excel(sheet_name):
    # Removed. Google Sheet only storage.
...
def is_google_auto_sync_enabled():
    # Google Sheet only mode. We still need secrets for read/write.
...
def sync_single_excel_sheet_to_google_sheet(sheet_name):
    # Disabled in Google Sheet only mode.
    return False, "Disabled"
...
def load_google_sync_state():
    return {
        "dirty_sheets": [],
        "last_sync_ts": 0,
        "last_sync_time": "Disabled",
        "last_sync_status": "Disabled",
        "last_sync_message": "Google Sheet only storage. Auto sync disabled.",
        "last_change_time": "Disabled"
    }


def save_sync_state(state):
    # Sync state disabled in Google-only mode.
    return None
...
def sync_changed_sheets_to_google():
    # Disabled. Direct Google Sheet save/read only.
...
def processing_wait_3s(message="Processing entry"):
    hero_fullscreen_loader(message)
...
# ============================================================
def page_dashboard():
    if not require_hero_show("dashboard", "🚀 HERO SHOW DASHBOARD", "Loading Dashboard..."):
        return
...
# ============================================================
def page_attendance():
    if not require_hero_show("attendance", "📍 HERO SHOW ATTENDANCE", "Loading Attendance..."):
        return
...
def page_upload_invoice():
    if not require_hero_show("upload_invoice", "🚀 HERO SHOW OCR UPLOAD", "Loading OCR Upload..."):
        return
...
# ============================================================
def page_reports():
    if not require_hero_show("reports", "📋 HERO SHOW REPORTS", "Loading Reports..."):
        return
...
# ============================================================
def page_search():
    if not require_hero_show("search", "🔍 HERO SHOW SEARCH", "Loading Search..."):
        return
...
def page_customer_service_history():
    if not require_hero_show("history", "📖 HERO SHOW HISTORY", "Loading Customer History..."):
        return
...
def page_manual_invoice():
    if not require_hero_show("manual_bill", "🧾 HERO SHOW MANUAL BILL", "Loading Manual Bill..."):
        return
...
def page_delete_invoice_request():
    if not require_hero_show("delete_request", "🗑️ HERO SHOW DELETE REQUEST", "Loading Delete Request..."):
        return
...
def render_cloud_excel_view_only():
    ...
    <div class="cloud-view-panel">
        <h3>☁️ Google Sheet Sheet View Only</h3>
        <p>Admin Panel-kulla view only. Edit/Delete options illa. Google Sheet edit/delete panna Pull Google Sheet → Google Sheet use pannunga.</p>
    </div>
    """, unsafe_allow_html=True)

    if st.button("Refresh Google Sheet View", use_container_width=True, key="cloud_view_refresh_admin_tab"):
        st.rerun()
...
def page_cloud_excel_view_only():
    if not require_hero_show("cloud_view", "☁️ HERO SHOW VIEW", "Loading View Only..."):
        return
...
def page_admin_panel():
    if not require_hero_show("admin_panel", "⚙️ HERO SHOW ADMIN PANEL", "Loading Admin Panel..."):
        return
    ...
    with tabs[3]:
        ...
        <div class="pull-sync-panel">
            <h3>🔄 Pull Google Sheet → Google Sheet</h3>
            <p>Google Sheet-la direct edit/delete pannina, indha option use pannina app Google Sheet file update aagum.</p>
        </div>
        """, unsafe_allow_html=True)

        st.markdown("""
        <div class="pull-sync-warning">
            Warning: Pull pannina selected Google Sheet sheet Google Sheet data-kku match aagum.
            Google Sheet-la delete pannina rows Google Sheet-layum delete aagum.
        </div>
        """, unsafe_allow_html=True)
        ...
        if c_pull1.button("Pull Selected Google Sheet to Google Sheet", use_container_width=True):
            with st.spinner("Pulling selected Google Sheet data to Google Sheet..."):
        ...
        if c_pull2.button("Pull ALL Google Sheets to Google Sheet", use_container_width=True):
            ...
            if not confirm_pull:
                ...
                st.warning("Click same button again to confirm full pull. This overwrites Google Sheet sheets with Google Sheet data.")
            else:
                with st.spinner("Pulling all Google Sheet data to Google Sheet..."):
        ...
        st.caption("Manual sync removed. Google Sheet edit/delete panna Pull Google Sheet → Google Sheet use pannunga.")
...
def page_duplicate_upload_finder():
    if not require_hero_show("duplicates", "🔎 HERO SHOW DUPLICATES", "Loading Duplicate Finder..."):
        return

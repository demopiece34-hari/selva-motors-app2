...
def create_excel_if_missing():
    DATA_DIR.mkdir(exist_ok=True); UPLOAD_DIR.mkdir(exist_ok=True); PDF_DIR.mkdir(exist_ok=True); BACKUP_DIR.mkdir(exist_ok=True)
    ...
    try:
        client, err = get_google_client_primary(); sid=get_google_sheet_id()
        if client is not None and sid:
            ...
            for sheet_name, cols in SHEETS.items():
                ...
                if not values:
                    ...
                    headers=values[0]; missing=[c for c in cols if c not in headers]
                    if missing:
                        old_rows=values[1:]
                        df=pd.DataFrame(old_rows, columns=headers) if old_rows else pd.DataFrame(columns=headers)
                        for c in missing: df[c]=""
                        ordered=headers+missing
                        ws.clear(); ws.update([ordered]+df.fillna("").astype(str).values.tolist(), value_input_option="USER_ENTERED")
    except Exception: pass
...
def read_sheet(sheet_name):
    ...
    try:
        if EXCEL_FILE.exists():
            df=cached_local_excel_read(str(EXCEL_FILE), EXCEL_FILE.stat().st_mtime, sheet_name).copy()
        else:
            df=pd.DataFrame(columns=SHEETS.get(sheet_name, []))
    except Exception:
        df=pd.DataFrame(columns=SHEETS.get(sheet_name, []))
...
def write_sheet(sheet_name, df):
    ...
    if ok:
        local_backup_write(sheet_name, df)
        try: st.success("Google Sheet updated.")
        except Exception: pass
    else:
        try: st.error("Google Sheet update failed: "+str(msg))
        except Exception: pass
        local_backup_write(sheet_name, df)
    clear_storage_cache()
...
def append_row(sheet_name, row_dict):
    ...
    if ok:
        try: st.success("Saved directly to Google Sheet.")
        except Exception: pass
    else:
        try: st.error("Google Sheet save failed: "+str(msg))
        except Exception: pass
        try:
...
def page_dashboard():
    ...
    page_hero("Service Control Dashboard", "Google Sheet primary dashboard. Existing sheet data safe.", employee_name)
    invoices=clean_duplicate_columns(read_sheet("invoices"))
    for c in ["Total Amount","Labour Amount","Spare Amount"]:
        if c not in invoices.columns: invoices[c]=0
    ...
    with c1: metric_card("Today Revenue", f"₹{today_df['Total Amount'].sum():,.0f}" if not today_df.empty else "₹0", "Today")
    with c2: metric_card("Today Vehicle", len(today_df), "Your ID only" if not (is_admin() or is_manager()) else "All")
    with c3: metric_card("Labour Amount", f"₹{today_df['Labour Amount'].sum():,.0f}" if not today_df.empty else "₹0", "Today labour")
    with c4: metric_card("Google Sheet", "Direct", "Primary storage")
...
# ============================================================
def page_search():
    page_hero("Search Service Entries", "Search invoice, vehicle and job card entries safely.", st.session_state.get("employee_name", ""))
...
def page_admin_panel():
    page_hero("Admin Panel", "Excel Sheet control center: revenue, employees, delete requests, data manager and settings.", "Admin")

    premium_panel("Admin Control Center", "Clean admin access: reports, customer history, Excel sheet manager, employee control and settings.")
    ...
    with tabs[3]:
        st.markdown("<div class='admin-tab-note'>Excel Status: Excel data Excel Sheet-ku auto sync 3 minutes once. Manual sync button also available.</div>", unsafe_allow_html=True)
        ...
        <div class="sync-status-box">
            ...
            Excel Sheets: {len(dirty_sheets)}<br>
            Storage: {wait_text}<br>
            Last Status: {state.get("last_sync_time", "Not yet")}
        </div>
        """, unsafe_allow_html=True)

        if is_google_sync_configured():
            st.success("Direct Excel sheet storage active.")
        else:
            st.warning("Direct Excel sheet storage active. Cloud sync disabled.")

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Storage Mode", "Direct")
        c2.metric("Excel Sheets", len(dirty_sheets))
        c3.metric("Storage", wait_text)
        c4.metric("Last Status", state.get("last_sync_time", "Not yet"))

        if dirty_sheets:
            st.caption("Waiting sheets: " + ", ".join(dirty_sheets))
        else:
            st.caption("No pending changes. Excel Sheet is updated or no changes made.")
        ...
        <div class="pull-sync-panel">
            <h3>🔄 Pull Excel Sheet → Excel Sheet</h3>
            <p>Excel Sheet-la direct edit/delete pannina, indha option use pannina app Excel Sheet file update aagum.</p>
        </div>
        """, unsafe_allow_html=True)

        st.markdown("""
        <div class="pull-sync-warning">
            Warning: Pull pannina selected Excel Sheet sheet Excel Sheet data-kku match aagum.
            Excel Sheet-la delete pannina rows Excel Sheet-layum delete aagum.
        </div>
        """, unsafe_allow_html=True)

        pull_sheet = st.selectbox(
            "Select Sheet to Pull from Excel Sheet",
            list(SHEETS.keys()),
            key="pull_google_sheet_name"
        )

        c_pull1, c_pull2 = st.columns(2)
        if c_pull1.button("Pull Selected Excel Sheet to Excel Sheet", use_container_width=True):
            with st.spinner("Pulling selected Excel Sheet data to Excel Sheet..."):
        ...
        if c_pull2.button("Pull ALL Excel Sheets to Excel Sheet", use_container_width=True):
            ...
            if not confirm_pull:
                ...
                st.warning("Click same button again to confirm full pull. This overwrites Excel Sheet sheets with Excel Sheet data.")
            else:
                with st.spinner("Pulling all Excel Sheet data to Excel Sheet..."):
        ...
        st.caption("Manual sync removed. Excel Sheet edit/delete panna Pull Excel Sheet → Excel Sheet use pannunga.")
    ...
    with tabs[5]:
        st.markdown("<div class='admin-tab-note'>Settings and password-protected Excel sheet download.</div>", unsafe_allow_html=True)

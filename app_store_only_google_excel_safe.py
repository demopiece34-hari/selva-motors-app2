...
# HERO WAITING LOADER
# ============================================================
def hero_waiting_loader(message="Saving entry to Excel sheet", seconds=3):
    box = st.empty()
    for remaining in range(seconds, 0, -1):
        box.markdown(f"""
        <div style="padding:22px;border-radius:24px;background:#ffffff;border:1px solid #e5e7eb;
                    box-shadow:0 10px 28px rgba(15,23,42,.10);text-align:center;margin:12px 0;">
            <div style="font-size:30px;font-weight:900;color:#dc2626;letter-spacing:1px;">HERO</div>
            <div style="font-size:17px;font-weight:800;color:#111827;margin-top:6px;">{message}</div>
            <div style="font-size:14px;color:#4b5563;margin-top:8px;">Please wait... {remaining} sec</div>
            <div style="margin-top:12px;height:8px;background:#f3f4f6;border-radius:999px;overflow:hidden;">
                <div style="height:8px;width:{int(((seconds-remaining+1)/seconds)*100)}%;background:#dc2626;border-radius:999px;"></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        time.sleep(1)

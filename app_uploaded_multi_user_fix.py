...
# Multi-user safety: prevents Excel read/write clash when many users use the app at same time.
EXCEL_LOCK = threading.RLock()
MULTI_USER_LOGIN_FIX_APPLIED = True
...
def locked_excel_read(sheet_name):
    """
    Multi-user safe Excel read.
    Prevents reading while another user is writing.
    """
    with EXCEL_LOCK:
        return pd.read_excel(EXCEL_FILE, sheet_name=sheet_name, engine="openpyxl")


def locked_excel_write(all_sheets):
    """
    Multi-user safe Excel write.
    Writes to a temporary file first, then replaces the main Excel file.
    This reduces corruption/read-write clash in Streamlit multi-user usage.
    """
    with EXCEL_LOCK:
...
def read_sheet(sheet_name):
    ...
    try:
        df = locked_excel_read(sheet_name)
    except Exception:

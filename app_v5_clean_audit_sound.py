...
def auto_save_attendance_from_gps(lat, lon):
    ...
    append_row("attendance", attendance_row)
    add_audit_log("Attendance Marked", "attendance", user_id, "", attendance_row)
...
def generate_manual_bill_pdf(customer_name, reg_no, bike_model, spare_rows, labour_amount, service_type, payment_mode="Cash", remarks=""):
    ...
    add_audit_log("Manual Bill Generated", "manual_invoices", bill_id, "", {"Registration Number": reg_no, "Service Type": service_type, "Total Amount": total_amount})
...
def play_welcome_sound():
    ...
    try {
        ...
        tone(523.25, 0.00, 0.18, 0.08);
        tone(659.25, 0.17, 0.18, 0.08);
        tone(783.99, 0.34, 0.24, 0.07);
    } catch(e) {}
    </script>
    """, unsafe_allow_html=True)
    ...
    with c2:
        ...
        if st.button("Login", use_container_width=True):
            ...
            if user:
                ...
                st.session_state["welcome_sound_pending"] = True
...
def add_audit_log(action, sheet_name="", reference_id="", old_value="", new_value=""):
    try:
        row = {
            ...
            "Old Value": str(old_value)[:500],
            "New Value": str(new_value)[:500],
        }
        df = read_sheet("audit_log")
        df = pd.concat([df, pd.DataFrame([row])], ignore_index=True)
        write_sheet("audit_log", df)
    except Exception:
        pass
...
def page_dashboard():
    ...
    st.markdown("<div class='theme-note'><b>Pro ERP Alert Center</b><br>Live service alerts, today entries and approval status.</div>", unsafe_allow_html=True)
    pro_alert_bar([
        ...
        if pending.empty:
            ...
            for idx, row in pending.iterrows():
                ...
                if c1.button("Approve Delete", key=f"approve_{idx}", use_container_width=True):
                    ...
                    st.cache_data.clear()
                    add_audit_log("Delete Request Approved", "delete_requests", str(row["Request ID"]), "", str(row["Entry ID"]))
                ...
                if c2.button("Reject Request", key=f"reject_{idx}", use_container_width=True):
                    ...
                    write_sheet("delete_requests", req)
                    st.cache_data.clear()
                    add_audit_log("Delete Request Rejected", "delete_requests", str(row["Request ID"]), "", str(row["Entry ID"]))
...
def main():
    ...
    if st.session_state.pop("welcome_sound_pending", False):
        play_welcome_sound()

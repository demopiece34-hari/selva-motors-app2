...
def generate_manual_bill_pdf(customer_name, reg_no, bike_model, spare_rows, labour_amount, service_type):
    ...
    c.roundRect(summary_x, y - 82, 225, 86, 12, fill=True, stroke=False)

    c.setFillColor(colors.HexColor("#0F172A"))
    c.setFont("Helvetica-Bold", 9)
    c.drawString(summary_x + 14, y - 18, f"Spare Amount: Rs.{spare_total:.2f}")
    c.drawString(summary_x + 14, y - 38, f"Labour Amount: Rs.{labour_amount:.2f}")

    c.setFillColor(colors.HexColor("#E31837"))
    c.setFont("Helvetica-Bold", 13)
    c.drawString(summary_x + 14, y - 64, f"Total Amount: Rs.{total_amount:.2f}")
...
def make_circle_hero_logo_for_pdf(size=190):
    """
    Creates a circular Hero logo PNG for professional PDF headers.
    Uses embedded/logo helper. Falls back safely if PIL/image fails.
    """
    try:
        logo_b64 = get_hero_logo_base64()
        if not logo_b64:
        ...
        # White square canvas with logo fitted inside
        canvas_img = _Image.new("RGBA", (size, size), (255, 255, 255, 255))

        # Fit logo into circle but keep enough white area
        inner = int(size * 0.78)
        logo_fit = _ImageOps.contain(img, (inner, inner))
        ...
        border.ellipse((3, 3, size - 3, size - 3), outline=(227, 24, 55, 255), width=8)

        out_path = PDF_DIR / "hero_circle_logo_pdf.png"
...
def draw_pdf_header(c, w, h, title, subtitle="", right_line1="", right_line2=""):
    ...
    if logo_path:
        c.drawImage(logo_path, 34, h - 92, width=58, height=58, mask="auto")
    else:
        c.setFillColor(colors.white)
        c.circle(63, h - 63, 29, fill=True, stroke=False)
        c.setFillColor(colors.HexColor("#E31837"))
        c.setFont("Helvetica-Bold", 14)
        c.drawCentredString(63, h - 68, "HERO")

    # Company block - single clean placement
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 18)
    c.drawString(105, h - 45, "SELVA MOTORS")

    c.setFont("Helvetica", 8)
    c.setFillColor(colors.HexColor("#E5E7EB"))
    c.drawString(105, h - 61, "KATCHANAM MAIN ROAD, KILVELUR")
    c.drawString(105, h - 76, subtitle or "Authorised Hero Service Document")
...
def draw_pdf_footer(c, w):
    ...
    c.setFillColor(colors.HexColor("#64748B"))
    c.setFont("Helvetica", 8)
    c.drawRightString(w - 38, 32, "Generated by Selva Motors ERP")

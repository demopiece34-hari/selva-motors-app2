...
# ============================================================
PDF_LIGHT_HEADER = colors.HexColor("#FFFFFF")
PDF_LIGHT_HEADER_STROKE = colors.HexColor("#FCA5A5")
PDF_HERO_RED = colors.HexColor("#DC2626")
PDF_TEXT_DARK = colors.HexColor("#1F2937")
PDF_TEXT_MUTED = colors.HexColor("#6B7280")
PDF_TABLE_HEADER = colors.HexColor("#FFF1F2")
PDF_ROW_ALT = colors.HexColor("#FFFDFD")


def draw_pdf_light_hero_header(c, w, h, title, subtitle="", right_line1="", right_line2=""):
    """
    Fully light print-friendly PDF header with big HERO text.
    No dark heavy ink background.
    """
    c.setFillColor(PDF_LIGHT_HEADER)
    c.roundRect(30, h - 98, w - 60, 78, 14, fill=True, stroke=False)
    c.setStrokeColor(PDF_LIGHT_HEADER_STROKE)
    c.setLineWidth(1.0)
    c.roundRect(30, h - 98, w - 60, 78, 14, fill=False, stroke=True)

    # Big HERO text logo - light background, minimal ink
    c.setFillColor(PDF_HERO_RED)
    c.setFont("Helvetica-Bold", 34)
    c.drawString(48, h - 53, "HERO")

    c.setFillColor(PDF_TEXT_DARK)
    c.setFont("Helvetica-Bold", 13)
    c.drawString(48, h - 76, "SELVA MOTORS")

    c.setFillColor(PDF_TEXT_MUTED)
    c.setFont("Helvetica", 8)
    c.drawString(48, h - 89, "Professional Service ERP")

    c.setFillColor(PDF_TEXT_DARK)
    c.setFont("Helvetica-Bold", 14)
    c.drawRightString(w - 48, h - 52, str(title))

    c.setFillColor(PDF_TEXT_MUTED)
    c.setFont("Helvetica", 9)
    if subtitle:
        c.drawRightString(w - 48, h - 68, str(subtitle))
    if right_line1:
        c.drawRightString(w - 48, h - 82, str(right_line1))
    if right_line2:
        c.drawRightString(w - 48, h - 94, str(right_line2))


def set_pdf_light_table_header(c, x, y, width, height=20):
    """
    Fully light table header for all PDFs.
    """
    c.setFillColor(PDF_TABLE_HEADER)
    c.roundRect(x, y, width, height, 6, fill=True, stroke=False)
    c.setStrokeColor(PDF_LIGHT_HEADER_STROKE)
    c.setLineWidth(0.6)
    c.roundRect(x, y, width, height, 6, fill=False, stroke=True)
    c.setFillColor(PDF_TEXT_DARK)
...
def generate_report_pdf(df, title, file_name):
    ...
    if df.empty:
        ...
        s_table.setStyle(TableStyle([
            ...
            ("TEXTCOLOR", (0, 0), (-1, 0), PDF_TEXT_DARK),

...
def create_excel_if_missing():
    """
    Existing Excel data safe:
    - Existing workbook overwrite panna koodathu.
    - Existing rows delete/change panna koodathu.
    - Missing sheets/columns mattum add aagum.
    """
    DATA_DIR.mkdir(exist_ok=True)
    UPLOAD_DIR.mkdir(exist_ok=True)
    PDF_DIR.mkdir(exist_ok=True)
    ...
    write_sheet("employees", default_employees)
    ...
    write_sheet("settings", default_settings)
...
def write_sheet(sheet_name, df):
    ...
    try:
        if False and "mark_sheet_dirty" in globals():
...
def append_row(sheet_name, row_dict):
    """
    Store-only append. App only stores new entries.
    Existing Excel rows are not affected.
    """
    df = read_sheet(sheet_name)
    if df.empty and sheet_name in SHEETS:
